...
model: haiku
tools: Read, Glob, Grep, Bash, mcp__playwright__browser_navigate, mcp__playwright__browser_navigate_back, mcp__playwright__browser_click, mcp__playwright__browser_type, mcp__playwright__browser_press_key, mcp__playwright__browser_snapshot, mcp__playwright__browser_take_screenshot, mcp__playwright__browser_hover, mcp__playwright__browser_select_option, mcp__playwright__browser_wait_for, mcp__playwright__browser_tabs, mcp__playwright__browser_close, mcp__playwright__browser_reload, mcp__playwright__browser_evaluate, mcp__playwright__browser_console_messages, mcp__playwright__browser_network_requests
---
...
- Settings에서 테마 토글 → `mcp__playwright__browser_snapshot`으로 root element `dark` class 확���
- `tp-*` CSS 변수가 light/dark에 따라 바뀌는지 검증
- `mcp__playwright__browser_evaluate`로 `document.documentElement.classList.contains('dark')` 확인 가능
...
- `mcp__playwright__browser_console_messages`: JavaScript 콘솔 에러 확인
- `mcp__playwright__browser_network_requests`: WebSocket 연결 실패 디버깅
- `mcp__playwright__browser_evaluate`: localStorage 상태 확인, DOM 검사

## Timeout 대처

- `mcp__playwright__browser_navigate` 타임아웃 → 1회 재시도, 실패 시 INCONCLUSIVE
- `mcp__playwright__browser_snapshot` 빈 결과 → 5초 대기 후 재시도
