...
- **Required status checks** (Ruleset 14604664): `lint`/`type-check`/`test`/`build-cli` 가 필수 merge gate. `rust` 와 `swift-build` 는 main에서 안정적으로 green 확인 후 required 목록에 추가한다 (없는 context를 required로 걸면 모든 PR이 wedge — 그래서 신규 job 은 항상 non-required 로 먼저 들어간다). **merge method = squash-only** (ruleset 강제 — 3종 merge 중 squash만 허용).
