...
- CI: Node 22 + Bun 1.3.13 + pnpm, 5개 독립 병렬 job (`lint`, `type-check`, `test`, `build-cli`, `rust`).
  ...
  - `type-check` job은 `packages/protocol` 포함 5개 tsconfig 병렬.
    ...
    - **Biome lint 심각도 = `biome.json` `linter.rules` SoT** — `suspicious.noExplicitAny = "error"` (T4a; 프로덕션 코드에 `any` 0개, 테스트 2곳은 `biome-ignore` 처리됨 → hard gate). `style.noNonNullAssertion` 은 아직 `warn` (프로덕션 ~33곳 정리 후 error 로 ratchet 예정 = T4b). `biome ci .` 는 error 에서만 non-zero exit — warn/info 는 CI 통과.
