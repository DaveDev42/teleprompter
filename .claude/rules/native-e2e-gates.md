...
# 실-Claude / 실-daemon E2E 게이트 (TP_E2E_*) — SoT

`.claude/rules/native-testing.md` 에서 분리된 real-Claude/real-daemon E2E 게이트 SoT. 마커 테이블·플랫폼 매트릭스·UI E2E(XCUITest)·서브커맨드 등 하니스 일반론은 `native-testing.md` 가 SoT 유지. 전부 **로컬 전용** — CI(`swift-smoke-ios`)에서 절대 돌지 않는다 (`.claude/rules/ci-workflows.md`).
...
대체; 그 TS 파일은 PR6 에서 삭제됨) 가 (1) 빈 포트에
...
- **입력 = 앱의 auto-probe** (the genuine app path, SMOKE-ONLY): holder 가 REPL 을 idle 로 만든 *뒤*, 앱이
  ...
  으로 주입됨). daemon relay-manager 가 chat 입력에 **`\r`(carriage return)** 을 붙여(`rust/tp-daemon/src/transport/relay_manager.rs`
  `build_events` 의 `on_input` 클로저, `InputKind::Chat` 분기 — TS 시절 `relay-manager.ts` `onInput`,
  `kind==="chat"` 포팅) PTY 로 제출한다 — **인터랙티브 claude TUI 는 `\r` 에만 프롬프트를 submit하고
  `\n` 으로는 입력 박스에만 남고 제출되지 않는다** (daemon→runner→PTY 전 경로로 경험적 검증: `text+\r` →
  `UserPromptSubmit`+`Stop`; `\n`(glued/separate) → 둘 다 0). 이게 **프로덕션 dogfood-chat 버그 수정**이다 —
  앱이 보낸 chat 메시지가 실제로 claude 에 제출되게 한다. 회귀 가드: 옛 `relay-manager.test.ts` "onInput →
  runner" 는 PR6 삭제 cascade 로 사라졌고, `relay_manager.rs`/`relay_client.rs` 유닛 테스트에도 이 동작을
  직접 pin 하는 케이스는 없다 — 현재 살아있는 커버리지는 아래 real-claude M5 E2E 게이트(`assert_m5_input`,
  격리 세션 DB 의 `UserPromptSubmit≥1` 확인)뿐이다.
...
- **왜 기존 E2E 로는 안 터지나 (둘 다 실측 확인)**: (1) `token_count()`/`on_record` 의 `tokenCount===0` 게이트
  (`rust/tp-daemon/src/push/notifier.rs`, TS 시절 `push-notifier.ts` 포팅) — Sim/macOS 는
...
- **관련 결정론 게이트**: `packages/daemon/src/session/runner-parity.test.ts` — Bun↔Rust differential
  ...
  (`TP_RUNNER_PARITY_REAL_CLAUDE=1`, 실 claude 로 hello/bye byte-exact mod {pid,ts} + io 구조 diff)였으나
  **Bun 툴체인과 함께 PR6(#5 zero-Bun cascade) 에서 삭제됐다.** 살아남은 증거는 이 섹션(위)의 **로컬
  실-claude `TP_E2E_RUNNER_BIN=1` 게이트** + `cargo test -p tp-daemon` + tp-core 골든벡터다.
...
- **정직한 범위**: 이 게이트는 파리티를 **증명**한다 — pre-flip 증명 도구였고 flip 후에도 회귀 가드로 유지.
  **기본 daemon default flip 완료 (task #4)**: Rust `tp` 의 background auto-spawn(`ensure_daemon.rs`
  ...
  구동, 5 macOS-native 런) 통과로 flip. (Bun SEA 블롭 `tpd` 자체는 #5 PR6 에서 삭제됐다 — 릴리즈 tarball 이
  싣는 `libexec/tp/tpd` 는 pre-PR6 설치본의 self-update 호환용 작은 `sh` stub 뿐이고, 현재 CLI 의 어떤 경로도
  그걸 exec 하지 않는다, `scripts/build-bundle.sh`.)
