...
- **`session.export` `limit` 은 non-positive 를 dispatcher 에서 즉시 거부한다**: `session.export` is not a `tp-proto` `IpcMessage`/`RelayClientMessage` variant at all — it is a relay-control message the daemon dispatches straight off the untyped `Value` in `rust/tp-daemon/src/ipc/command_dispatcher.rs` `handle_relay_session_export`. The handler reads `msg.get("limit").and_then(Value::as_i64)` and **rejects `limit <= 0` up front with an `err BAD_REQUEST` reply** before any store access — without this gate, `effective_limit + 1` would map e.g. `-2` to a SQLite `LIMIT -1`, which SQLite treats as **unlimited**, silently bypassing the 50k export cap (the exact bypass the old TS `isOptionalPositiveInt` guard existed to close). Positive values then flow through the upper-bound clamp `limit.unwrap_or(50_000).min(50_000)` → `RecordsFilter.limit = Some(effective_limit + 1)` → `SessionDb::get_records_filtered` (`rust/tp-daemon/src/store/session_db.rs`), which re-clamps identically and binds the SQL `LIMIT ?`. 회귀 가드: `command_dispatcher.rs relay_session_export_rejects_non_positive_limit` (-2/-1/0 → `err BAD_REQUEST`, no export).
- **Pairing deep-link 디코더는 strict 하다** (`rust/tp-core/src/pairing.rs` `decode_binary_pairing`). (1) base64url `d` 페이로드는 디코딩 전에 **2048 char 로 pre-cap** (`MAX_PAIRING_B64_LEN`, `pairing.rs:28,194`) — `b64url_decode` alloc 전에 거부해 attacker-controlled allocation 을 bound. 정상 v2/v3/v4 번들은 ~772 char 라 구조적으로 유효한 입력은 절대 cap 을 넘지 못한다. (2) did/relay/hostname 필드는 **strict UTF-8** (`std::str::from_utf8`, `pairing.rs:221,237,282`) — 실패 시 lossy substitution 없이 즉시 `err()` 로 전체 프레임을 거부한다. 회귀 가드: `pairing.rs` `rejects_over_cap_payload_before_decode`(cap+1 / 10×cap 거부 + 정상 번들 headroom 어서션) + `rejects_invalid_utf8_in_string_fields`(did/relay/hostname 각각 0xff 0xfe 주입 거부 + 동일 레이아웃 valid-UTF-8 대조군 통과).
...
- `relay.push` — daemon → relay request to send an APNs push to a target frontend. **`sealed` is REQUIRED** (there is no legacy plaintext `token` field on this wire union — `rust/tp-proto/src/relay_client.rs` `Push` variant). Carries `frontendId`, `sealed` (blob from daemon's store), `title`, `body`, optional `interruptionLevel`, optional `data`. Relay unseals the token (tries current key then prev key for rotation), then POSTs to APNs HTTP/2 using ES256 JWT auth (`rust/tp-relay/src/apns.rs`). **Error handling:** unseal failure → `relay.err PUSH_UNSEAL_FAILED`; APNs 400 `BadDeviceToken` or 410 `Unregistered` → `relay.err PUSH_TOKEN_DEAD`; non-dead APNs errors → `relay.err PUSH_DELIVERY_ERROR`. The `relay.err` wire type (`rust/tp-relay/src/messages.rs` `RelayErr`) is `{ e: String, m: Option<String>, frontendId: Option<String> }` — `frontendId` is **additive-optional** (serde `rename = "frontendId"`, `default` + omit-when-`None`; old daemons ignore the extra field, old relays simply omit it — WS additive-field 원칙). The relay populates it on the push-scoped codes (`PUSH_UNSEAL_FAILED` in `conn.rs handle_push`; `PUSH_RATE_LIMITED`/`PUSH_DELIVERY_ERROR`/`PUSH_TOKEN_DEAD` in `conn.rs map_delivery_result`) and leaves it `None` on connection-scoped codes (`RATE_LIMITED`, `FRAME_TOO_LARGE`, …). The daemon's `handle_relay_err` (`rust/tp-daemon/src/transport/relay_client.rs`) **evicts the stored push token per-frontend** when `frontendId` is present on `PUSH_UNSEAL_FAILED`/`PUSH_TOKEN_DEAD` (via the `on_push_unseal_failed`/`on_push_token_dead` callbacks → `PushNotifier::handle_unseal_failed`/`handle_token_dead`); when absent (old relay) it falls back to the legacy self-heal-on-reconnect behavior and must NEVER guess an eviction target (e.g. by parsing the human-readable `m` string). 회귀 가드: relay 측 `messages.rs relay_err_frontend_id_is_additive_optional` + `conn.rs map_delivery_result_errors_reply_to_daemon`(structured fid 어서션 포함), daemon 측 `relay_client.rs relay_err_with_frontend_id_evicts_dead_and_unsealable_tokens` + `relay_err_without_frontend_id_does_not_evict`. The `interruptionLevel` field (`"active" | "time-sensitive"`; absent → `"active"`) maps to APNs `aps.interruption-level`; for `time-sensitive` only, the relay also sets the `apns-priority: 10` header so APNs cannot defer the push (`rust/tp-relay/src/apns.rs:515-517`; regression guards `time_sensitive_sets_priority_10` / `non_time_sensitive_omits_priority`). The privileged `"critical"` level is intentionally **not** modeled — the wire guard (`InterruptionLevel::parse_opt`, `relay_client.rs`) rejects any other string. **Per-event differentiation lives in the daemon** (`rust/tp-daemon/src/push/notifier.rs` `interruption_level_for`, line 55). APNs environment (`sandbox` vs `prod`) is configured per-deployment via `APNS_ENV` — not on the wire.
