...
**Dead APNs token eviction — 양측 wired.** APNs가 HTTP 400 `BadDeviceToken` 또는 410 `Unregistered` 를 반환하면 relay(`conn.rs` `map_delivery_result`)는 daemon에 `relay.err { e: "PUSH_TOKEN_DEAD", m: "…", frontendId: "<fid>" }` 를 전송한다(unseal 실패의 `PUSH_UNSEAL_FAILED` 도 동일 shape — `conn.rs handle_push`). `RelayErr` wire struct(`messages.rs`)의 `frontendId` 는 **additive-optional 구조화 필드**다(serde `rename="frontendId"`, `None` 이면 wire 에서 생략 — 구 daemon 은 무시, 구 relay 는 미전송; WS additive-field 원칙). daemon 측 `RelayClient::handle_relay_err`(`rust/tp-daemon/src/transport/relay_client.rs`)는 `PUSH_UNSEAL_FAILED`/`PUSH_TOKEN_DEAD` 에 `frontendId` 가 실려 오면 `on_push_unseal_failed`/`on_push_token_dead` 콜백(→ `PushNotifier::handle_unseal_failed`/`handle_token_dead`, relay_manager.rs 배선)으로 **그 frontend 의 store token 을 즉시 evict** 한다. `frontendId` 부재(구 relay) 시에만 legacy 폴백 — eviction 없이 로그만 남기고 frontend 의 다음 relay 재연결 `relay.push.register` 재등록으로 self-heal 한다(`m` 문자열 파싱으로 타깃을 추측하는 것은 금지). Frontend 가 앱 재설치 또는 재연결 시에도 새 `relay.push.register`로 재등록한다. APNs JWT 토큰(`ApnsJwtSigner`)은 50분마다 자동 갱신된다. 회귀 가드 양측: relay 송신측 `conn.rs map_delivery_result_errors_reply_to_daemon`(올바른 daemon conn + structured `frontendId` 어서션) + `messages.rs relay_err_frontend_id_is_additive_optional`(present→parse, non-string→reject, None→wire 생략); daemon 수신측 `relay_client.rs relay_err_with_frontend_id_evicts_dead_and_unsealable_tokens` + `relay_err_without_frontend_id_does_not_evict`.
