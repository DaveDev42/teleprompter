...
| Target | Workflow | 설명 |
|--------|----------|------|
| tp 바이너리 | GitHub Actions `release.yml` | darwin-arm64 + linux × {arm64, x64} — `scripts/build-bundle.sh` 가 Rust 4-bin(`tp`,`tp-daemon`,`tp-relay`,`tp-runner`) prefix-tree 번들 `tp-<suffix>.tar.gz` 를 빌드 (PR6; 구 Bun 단일-바이너리 asset 드롭) → GitHub Release. `release.yml`은 `push: tags: [v*]` 와 `workflow_dispatch -f tag=...` 둘 다 받지만, GitHub API tag-creation 의 push event firing 이 누락되는 케이스가 잦아 (#172) **항상 manual dispatch 로 트리거** 한다. |
...
- **CLI**: GitHub Releases → Rust 번들 tarball (`scripts/build-bundle.sh`, darwin/linux × arm64/x64; PR6 에서 `bun build --compile` 대체. Windows users run the linux build under WSL)
