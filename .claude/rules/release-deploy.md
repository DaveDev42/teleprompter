...
- `apps/app/fingerprint.config.js`: **pod install 이 node_modules 안에 쓰는 파일들을 fingerprint 에서 제외** (`ignorePaths`). EAS iOS 빌더는 PREBUILD + INSTALL_PODS **이후** runtimeVersion 을 재계산해 workflow fingerprint job(클린 체크아웃) 값과 비교·검증하는데, pod install 시점 mutation 이 fingerprint 소스 dir 에 들어가면 두 값이 갈려 빌드가 거부된다 ("Runtime version mismatch"). 알려진 mutation: ① react-native-quick-crypto 가 `SODIUM_ENABLED=1` 일 때 libsodium 소스를 자기 패키지 dir (`ios/libsodium-stable/`)에 추출, ② expo-modules-autolinking precompiled flow 가 prebuilt xcframework 제공 라이브러리(reanimated/screens/safe-area-context/worklets)의 패키지 dir 에 패치된 `<PodName>.podspec.json` 을 생성 — 원격 아티팩트가 CDN 에 존재할 때만 발동하므로 **외부 요인으로 갑자기 나타날 수 있다** (2026-06: reanimated 4.3.1×RN 0.85.3 아티팩트가 CDN 에 올라오면서 이전엔 통과하던 빌드가 깨짐; 이 flow 는 2026-06-13 부로 영구 비활성 — 강제선은 `package.json` `expo.autolinking.ios.buildFromSource: [".*"]` (app.json `usePrecompiledModules: false` 만으로는 EAS worker 가 ENV 를 직접 export 해 우회됨, 빌드 #72 사고). prebuilt ABI mismatch 런치 크래시, `.claude/rules/native-build.md` 참조. ignorePaths 는 방어적으로 유지), ③ react-native-audio-api 의 RNAudioAPI.podspec `prepare_command` 가 pod install 중 `scripts/download-prebuilt-binaries.sh` 로 prebuilt 오디오 바이너리를 자기 패키지 dir (`common/cpp/audioapi/external/{iphoneos,iphonesimulator}/`)에 다운로드. 새 네이티브 의존성이 pod install 중 자기 패키지 dir 에 파일을 쓰면 이 config 에 패턴을 추가할 것. 패턴은 반드시 `**/` 로 시작해야 monorepo 의 `../../node_modules/...` 소스 경로에 매칭된다.
