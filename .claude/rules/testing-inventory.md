...
- `rust/tp-cli/src/locate.rs` `#[cfg(test)] mod tests` (`cargo test -p tp-cli`) — `locate_bun_blob` (tpd) + `locate_tp_daemon` (ADR-0003 Phase 4 flip-prep A1) + `locate_tp_relay` (task #17 #25) 리졸버 유닛. tp-daemon/tp-relay 케이스 각 4종: prefix-tree(`<prefix>/libexec/tp/tp-{daemon,relay}`)·sibling·**dev-fallback 이 `dist/` 아닌 `rust/target/release/tp-{daemon,relay}`**(cargo bin) geometry + not-found 에러가 `TP_{DAEMON,RELAY}_BIN`+`Reinstall` 명시. `locate_tp_daemon()`은 A1 시점 `#[allow(dead_code)]` (flip PR 이 첫 실 caller — 그때 allow 제거); `locate_tp_relay()`는 #25 에서 `commands::relay::run` 이 첫 실 caller 라 allow 없이 wired. 아티팩트 라운드트립(tar 멤버 존재 + 실행가능 Mach-O)은 게이트 아닌 릴리즈 검증.
