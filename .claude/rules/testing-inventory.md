...
- `packages/daemon/src/worktree/worktree-rust-parity.test.ts` — **Bun `WorktreeManager` ↔ Rust `tp-daemon` differential worktree 파리티 게이트** (ADR-0003 Phase 4 daemon inc2). git worktree 는 shared on-disk artifact 가 아니므로 store 게이트와 달리 **differential behavior gate**: 같은 op 시퀀스(add/list/remove/containment-reject)를 두 impl 에 **독립된 sibling `git init` repo** 로 각각 돌려 결과가 structurally identical 인지 assert — add/list 구조 + main-first 순서 동일, **escape path(`../../../etc/evil`)가 `add`/`remove` 양쪽에서 동일 boundary 에러로 REJECT**(trust-boundary write-escape 가드가 두 impl 에서 일치). 비결정 필드(`path` 절대경로/`head` SHA)는 비교 전 normalize(store 게이트의 created_at/updated_at strip 과 동일 규율 — 두 repo 가 독립이라 절대경로/SHA 는 정당하게 다름). Rust 는 store 게이트와 **같은 `tp-daemon-probe`** 의 worktree verbs(`worktree-add`/`worktree-list`/`worktree-remove`, `<repoRoot>` 를 arg[1] 로 — store 를 안 열고 dispatch)로 driving. **probe 미빌드 시 SKIP** (store 게이트와 동일 binary — `test` job 의 `cargo build --release --bin tp-daemon-probe` 가 둘 다 상시화). worktree 보안 로직(containment/gitEnv-strip/porcelain)은 `tp-daemon` lib unit test(`cargo test -p tp-daemon --lib`, 실 git init tempdir)가 이미 pin — 이 게이트는 cross-impl 일치를 추가로 증명.
- `packages/daemon/src/transport/relay-client-rust-parity.test.ts` — **Bun↔Rust relay-client reconnect-policy differential 파리티 게이트** (ADR-0003 Phase 4 daemon inc3). relay-client 의 load-bearing pure core (`computeReconnectPlan(attempt, peerless)` + `nextPeerlessReconnects(current, hadPeer)`, relay-client.ts:95/122 — dead-pairing throttle 의 핵심)를 두 impl 에 **같은 (attempt × peerless) + (current × hadPeer) 그리드**로 돌려 출력 byte-identical 검증: exponential backoff `1000*2^attempt` + `RECONNECT_MAX_MS`(30s) cap + `MAX_RECONNECT_ATTEMPT` clamp + **throttle(peerless≥3 → 30min, attempt UNCHANGED)** + counter arm(+1)/reset(0) 전 branch. Rust 는 store/worktree 게이트와 **같은 `tp-daemon-probe`** 의 verbs `reconnect-plan <attempt> <peerless>`(→`{delayMs,nextAttempt}`)·`peerless-next <current> <hadPeer:0|1>`(→`{value}`, store 안 열고 dispatch)로 driving. **probe 미빌드 시 SKIP** (동일 binary — `test` job 의 `cargo build --release --bin tp-daemon-probe` 가 세 게이트 모두 상시화). relay-client 의 full state machine (kx/resume/dispose-race/E2EE seal) 은 `cargo test -p tp-daemon`(88 pass) lib unit + tp-core golden-vector 가 커버 — 이 게이트는 storm-safety 로직(throttle/backoff)의 cross-impl 일치를 추가로 증명 (drift 위험 최고 지점).
- `packages/daemon/src/push/push-notifier-rust-parity.test.ts` — **Bun↔Rust push-gate differential 파리티 게이트** (ADR-0003 Phase 4 daemon inc4). `PushNotifier` 의 load-bearing 결정 (NOTIFY_EVENTS 게이트 + tokenCount>0 게이트 + `interruptionLevelFor` + `buildPushMessage` title/body)을 두 impl 에 **같은 (eventName × tokenCount × payload) 그리드**로 돌려 출력 byte-identical 검증: notify-eligible 3종 + non-notify(Stop)/unknown 억제 + tokenCount==0 억제 + Notification title regex arm(permission/wait-idle/generic) + PermissionRequest tool/no-tool + Elicitation message/question/empty + **code-point-safe multibyte truncation**(🇰🇷×200 — Bun↔Rust 발산 함정) 전 branch (`shouldNotify`+`level`+`title`+`body` 전부 일치). Rust 는 store/worktree/reconnect 게이트와 **같은 `tp-daemon-probe`** 의 verb `push-gate <eventName> <tokenCount> [payloadJson]`(→`{shouldNotify,level,title,body}`, store 안 열고 dispatch)로 driving. **probe 미빌드 시 SKIP** (동일 binary — `test` job 의 `cargo build --release --bin tp-daemon-probe` 가 네 게이트 모두 상시화). `PushNotifier` full state (token register/purge/dead)·`RelayConnectionManager` (store-first removePairing/removingDaemonIds guard/rename)·pairing (rank-1 guard/cancel-during-connect) 은 `cargo test -p tp-daemon`(136 pass) lib unit 이 커버 — 이 게이트는 push 결정의 cross-impl 일치를 추가로 증명 (user-visible split 위험 지점).
- `packages/daemon/src/ipc/dispatcher-rust-parity.test.ts` — **Bun↔Rust command-dispatcher differential 파리티 게이트** (ADR-0003 Phase 4 daemon inc5). **dispatcher-level** (socket-level 아님 — 양쪽 다 in-process seam 을 driving, IPC framing 은 각자 ipc/server 테스트가 커버): 실 Bun `IpcCommandDispatcher` 와 실 Rust dispatcher(같은 `tp-daemon-probe` 의 verbs `sanitize-sid <label>`·`dispatch-bye <vault> <sid> <regPid|-> <byePid|-> <exitCode> <reason|->`·`dispatch-create-sid <vault> <sid>`·`dispatch-export <vault> <sid> <format> <limit|->`)를 같은 입력으로 돌려 reply frame·store side-effect·guard 결정 일치 검증 — **rank 3**(traversal sid 가 createSession/subscribe *전* 거부, 에러 메시지 byte-identical + 부작용 0), **rank 4**(exactly-limit → truncated:false off-by-one, json/markdown; markdown `d` 는 shared-vault 로 byte-exact), **bye truth table**(`reason:"signal"` 항상 stopped) + **stale-pid generation guard**, `sanitizeForSid` 그리드. 프레임은 parsed-object 비교(serde_json BTreeMap 알파벳 키순서 vs Bun 삽입순 — key order 는 wire-significant 아님); export 는 한 vault 공유(read-only)라 timestamp normalize 불요, bye 는 두 vault 동일 시드. **probe 미빌드 시 SKIP** (동일 binary). 이 게이트가 실제로 잡은 발산: `to_wire_session_meta` 가 absent worktreePath/claudeVersion 을 explicit null 로 emit (TS 는 `JSON.stringify` 가 undefined 키를 drop → 키 자체 생략) — Rust 측 수정으로 hello/state/export 프레임 byte-parity 복원.
...
- `rust/tp-cli/src/locate.rs` `#[cfg(test)] mod tests` (`cargo test -p tp-cli`) — `locate_bun_blob` (tpd) + `locate_tp_daemon` (ADR-0003 Phase 4 flip-prep A1) + `locate_tp_relay` (task #17 #25) + `locate_tp_runner` (task #8 flip-prep) 리졸버 유닛. tp-daemon/tp-relay/tp-runner 케이스 각 4종: prefix-tree(`<prefix>/libexec/tp/tp-{daemon,relay,runner}`)·sibling·**dev-fallback 이 `dist/` 아닌 `rust/target/release/tp-{daemon,relay,runner}`**(cargo bin) geometry + not-found 에러가 `TP_{DAEMON,RELAY,RUNNER}_BIN`+`Reinstall` 명시. **wired vs unwired 구분**: `locate_tp_relay()`는 #25 에서 `commands::relay::run` 이 첫 실 caller 라 allow 없이 wired; `locate_tp_daemon()`(A1)·`locate_tp_runner()`(task #8 flip-prep)는 `#[allow(dead_code)]` — flip PR 이 첫 실 caller(그때 allow 제거). runner/daemon 은 flip 이 E2E-soak-gated 라 아티팩트 shipping 을 default flip 과 분리(relay 는 아니라 same-PR wired). 아티팩트 라운드트립(tar 멤버 존재 + 실행가능 Mach-O)은 게이트 아닌 릴리즈 검증.
- `rust/tp-cli/src/commands/relay.rs` `#[cfg(test)] mod tests` (`cargo test -p tp-cli`, task #17 #25) — 네이티브 `tp relay …` 핸들러 유닛 8종: `parse_start_flags` 가 `--port` space/equals form·세 플래그 전부·unknown-flag 무시(Bun `strict:false` 파리티)·값 없는 플래그 unset 처리; `valid_int` 가 plain/trimmed 정수 accept + non-numeric/음수/소수/빈값 reject; `USAGE` 상수가 Bun `relayCommand` default arm(relay.ts:15-21) 과 byte-parity. subcommand 라우팅(start→exec, ping→doctor-msg, default→usage)·flag→argv/env 번역(`--cache-size`→`TP_RELAY_CACHE_SIZE`, `--max-frame-size`→`TP_RELAY_MAX_FRAME_SIZE`)·exec-replace 는 로컬 라이브 검증(fake tp-relay echo + prefix-tree 라운드트립).
