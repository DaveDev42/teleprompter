...
- **6개 Bun↔Rust differential 파리티 게이트는 삭제됨** (Bun-deletion cascade #5, PR4): `session/runner-parity.test.ts` + 5개 `*-rust-parity.test.ts`(store/worktree/transport/push/ipc) + 이들을 driving 하던 `tp-daemon-probe` bin. 이 게이트들은 Bun impl 이 존재하는 동안 Rust 포트의 byte-exactness 를 교차검증했지만, Bun 백엔드가 PR6 에서 삭제되므로 존속 이유가 사라졌다. 각 게이트가 커버하던 Rust 로직(store/worktree/reconnect-policy/push-gate/dispatcher)은 이제 `cargo test -p tp-daemon`(187 pass) lib unit + tp-core golden-vector 가 단독으로 커버한다. `tp-runner` 는 **출하 bin** 이라 유지(삭제된 건 그 differential 테스트뿐).
