...
- Chat UI is **hooks-only** (PTY-to-chat fallback removed in PR #457): hooks events render as structured message cards; the Stop event's `last_assistant_message` is the canonical response. PTY io records go exclusively to the Terminal tab. Tool events render as **one stateful card**: `SessionStore.appendRec` folds a `PostToolUse` into its pending `PreToolUse` item (matched by `tool_use_id`; merged item keeps the Pre's seq so SwiftUI flips running→done in place), and the real result key is `tool_response` (`tool_result` is the TS-era fallback).
