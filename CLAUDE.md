...
- `apps/app/eas.json`: `"appVersionSource": "local"` — `app.json`의 `expo.version`이 권위. EAS 서버가 별도로 버전을 저장하지 않음.
- `apps/app/app.json`: `"runtimeVersion": { "policy": "appVersion" }` — runtimeVersion = appVersion 문자열. `expo.version` bump 시에만 네이티브 재빌드 필요.
- 릴리즈 당 1회 네이티브 빌드 + submit (release-please PR이 `expo.version`을 올리므로). 같은 `0.1.x` 안에서 hotfix JS 변경은 `eas update --branch preview/production`로 OTA만 발행.
- `eas.json` 각 profile의 `autoIncrement: true`가 `ios.buildNumber` / `android.versionCode`를 증분 (human-readable version과 별개).
