# IN_PROGRESS — 후속 세션 인계 (2026-07-08 갱신 · rev22)
...
## 상태 스냅샷 (origin/main = 363aa9ee, 2026-07-08 rev22)

> rev22 (2026-07-08): **de-trampoline tp-cli(#17) — relay flip 머지(#905, main=`363aa9ee`) 이후 runner flip-prep(#8 의 기계적 선행) 코드+검증 완료, PR 대기.** relay flip(#905)으로 `run` 이 **blob 에 남은 유일 route**(`Route::Forward`)가 됐고, 그 native flip 은 #8 (E2E-soak-gated, 사용자 구동). 이번 작업은 daemon A1(#898) 패턴의 **충실한 클론** — `tp-runner` 를 release 아티팩트로 ship + `locate_tp_runner()` 추가, **default 는 아무것도 안 바꿈**(#8 flip 의 precondition).
> **runner flip-prep 상세 (branch 대기, base `363aa9ee`)**: (1) `rust/tp-cli/src/locate.rs` — `locate_tp_runner()` + `_inner`(`locate_tp_relay` 5-tier 클론: `$TP_RUNNER_BIN` env override[is_self 무한루프 가드] → `../../libexec/tp/tp-runner` release prefix → sibling → dev-fallback `rust/target/release/tp-runner`[cargo bin, NOT `dist/` blob] → hard-err naming `TP_RUNNER_BIN`+Reinstall), **둘 다 `#[allow(dead_code)]`** — #8 flip 이 첫 실 caller(daemon A1 `locate_tp_daemon` 미러, relay `locate_tp_relay` 처럼 same-PR wired 아님; runner flip 이 E2E-gated 라 shipping 을 default flip 과 분리). + 4 유닛(prefix-tree/sibling/dev-fallback/not-found). `TP_RUNNER_BIN` 은 TS seam(`runner-bin.ts resolveRunnerBinOverride`)과 **공유 escape hatch — coherent, 충돌 아님**(TS=daemon 이 per-session Rust runner spawn 여부, Rust locate=native flip 이 shipped runner 위치). (2) `scripts/build.ts` step 2d(`cargo build --release --bin tp-runner --target <t>` → `libexec/tp/tp-runner`) + header doc + Tree 로그. (3) `scripts/install.sh` guarded `[ -f … ]` cp + summary line + header. (4) `.github/workflows/ci.yml` bundle-smoke `ls libexec/tp/tp-runner`(forward-seam probe 불변 — `run` 은 #8 까지 blob 유지). (5) `.claude/rules/testing-inventory.md` locate_tp_runner 인벤토리.
> **검증**: `cargo test -p tp-cli` 310 pass(+4 runner, was 306) · `cargo clippy --workspace --all-targets` 0 deny-error(`#[allow(dead_code)]` 가 unwired-fn 억제; pre-existing pedantic doc-backtick warn 은 line 17 module-doc, 내 diff 밖) · `cargo fmt --check` clean · darwin-arm64 bundle 빌드로 `libexec/tp/tp-runner` = 실행가능 arm64 Mach-O tarball 멤버 확인(`parse_args` 가 sid 를 default 하므로 no-args 는 종료 아닌 세션 구동 — 실 runtime 증명). `bash -n install.sh`/build.ts/ci.yml syntax OK. **다음 = PR ship → dogfood refresh → #8 flip(사용자 E2E soak) 또는 #4(daemon flip)/#5(Bun backend 삭제).**

> rev21 (2026-07-08): **daemon Rust 포팅 — inc6 머지(#897) + 포트 ladder inc1–6 코드 완료. 이후 "flip 앞까지 전부 prep"(사용자 승인) — flip-prep A1 머지(#898, main=`158d0f22`), A2 코드+검증 완료 PR 대기. flip 자체(default 전환)만 사용자의 로컬 N× clean 실 claude E2E soak 뒤로 남음.**
> **A1 머지(#898)**: `tp-daemon` 을 release 아티팩트로 ship + `locate_tp_daemon()`. `scripts/build.ts` step 2b(`cargo build --release --bin tp-daemon --target <t>` → `libexec/tp/tp-daemon`), `scripts/install.sh` guarded `[ -f … ]` cp, `rust/tp-cli/src/locate.rs` `locate_tp_daemon()`(`locate_bun_blob` 5-step 미러, dev-fallback=`rust/target/release/tp-daemon`, **`#[allow(dead_code)]` 미배선** — 배선=flip). 5 required job GREEN(biome-format 1건 pre-push 훅에서 fix, same commit). dogfood 재빌드+재설치 완료(tp v0.1.52, daemon=`tpd` 트램폴린 pid 28558 running, brew symlink 무손상).
> **A2 (branch 대기, base `158d0f22`)**: `TP_E2E_DAEMON_BIN` daemon-parity 게이트 = `TP_E2E_RUNNER_BIN` 의 daemon 쌍둥이. `scripts/real-daemon-pair.ts`: `DAEMON_BIN`(reads `TP_DAEMON_BIN`) + `daemonCmd()`(Rust→`[<tp-daemon>]` 서브커맨드 없음, else `[bun,run,<cli>,daemon,start]`) 로 단일 하드코딩 spawn `cmd` 대체 + `DAEMON_PARITY_BIN=<path>` stdout emit. `scripts/ios.sh`: `E2E_DAEMON_BIN` 게이트(implies `E2E_REAL`, claude 게이트와 직교, `E2E_RUNNER_BIN` 과 조합 가능) + `build_rust_daemon_bin`(release→debug fallback, loud-die) + `TP_DAEMON_BIN="$REAL_DAEMON_BIN"` 리터럴 env-prefix 주입 + `assert_daemon_parity`(7 call site). **positive 증명이 PRIMARY**: Bun↔Rust daemon 이 설계상 byte-identical 이라 `grep -F DAEMON_PARITY_BIN=<path>` 가 유일 판별자(silent Bun fallback 방지) + `records≥1` store 체크 보조. holder-side 인 이유: holder 가 daemon 을 직접 spawn(≠`ensureDaemon`), foreground `tp daemon start` 는 override 미honor(inc6 scope).
> **검증**: `bash -n scripts/ios.sh` OK · `real-daemon-pair.ts` parse OK(bun build exit 0) · `biome check .` exit 0(신규 warning 0 — baseline diff 동일) · `type-check:all` exit 0. 게이트 자체는 로컬 전용 실 claude(절대 CI 아님). docs sync: native-testing.md(env-var row + 전용 섹션), daemon-rust-port-plan.md(A2 row + inc6 DEFERRED→delivered), IN_PROGRESS rev21. **다음 = A2 PR ship → dogfood refresh → 그다음 flip(#4, E2E-gated, 사용자 soak) 또는 #5(Bun backend 삭제).**

> rev20 (2026-07-08): **daemon Rust 포팅 — 증분 5 머지(#896, main=`18552ec4`) + 증분 6(`TP_DAEMON_BIN` opt-in dual-run seam, M) 코드+게이트 완료, PR 대기. 이로써 포트 ladder inc1–6 코드 완료 — 남은 건 별도 default-flip(E2E-gated, #4).**
> **inc5 머지**: dispatcher differential 게이트 CI GREEN(5 required job), `gh api` PUT squash(`18552ec4`). dogfood 재빌드+재설치 완료(tp v0.1.52, daemon=`tpd` 트램폴린 pid 79806 running, brew symlink 무손상).
> **inc6 (branch `feat/tp-daemon-inc6-daemon-bin`, base `18552ec4`)**: 러너 seam(`TP_RUNNER_BIN`) 을 그대로 미러한 데몬 seam.
> NEW `apps/cli/src/lib/daemon-bin.ts` `resolveDaemonBinOverride(env)` (unset/empty→null, `accessSync(X_OK)`, **무효면 throw** — 조용한 Bun fallback 절대 없음, `errorWithHints` cargo-build 힌트, trust boundary=daemon-spawn 프로세스 env 전용).
> seam 위치 = `ensure-daemon.ts` — 인라인 `[cmd, spawnArgs]` 선택을 pure export `resolveDaemonSpawnCommand(env)` 로 추출: override→`[<tp-daemon>, []]` (**`daemon start` 서브커맨드 없음** — Rust bin 이 곧 daemon: pid-lock→`Daemon::new/start`→auto-cleanup→signals, double-spawn=같은 pid-lock 으로 no-op exit-0), 없으면 기존 compiled/dev argv **byte-identical**. `ensureDaemon()` 이 try/catch 로 감싸 무효 override=fail-loud exit (runner seam wrap idiom). **socket parity 자동** — TS `getSocketPath()` ≡ Rust `tp-proto get_socket_path()` = `resolveRuntimeDir()/daemon.sock`, `--socket-path` flag 없음(미추가).
> **scope 결정 — foreground `tp daemon start` 미배선**: `new Daemon()` in-process 구성이라 override 존중=exec-into-Rust-bin(구조적으로 다른 op, flip PR 소관). soak 는 `tp-daemon` 직접 띄우면 socket+pid-lock parity 로 모든 `tp` 클라이언트가 라우팅되어 도달 가능. `service-*.ts`(launchd/systemd) 불변 — 서비스 daemon 은 flip 전까지 Bun. **`TP_E2E_DAEMON_BIN` ios.sh 게이트 DEFERRED**(holder-side 선택 필요, runner 선례).
> **게이트 = 플레인 always-on Bun unit test**(probe-driven 아님 — ci-workflows 5-게이트 카운트 불변): `daemon-bin.test.ts`(5: null/path/throw-missing+cargo-hint/throw-non-exec) + `ensure-daemon.test.ts` `resolveDaemonSpawnCommand` describe(4: absent==default byte-identical/empty fall-through/override `[bin,[]]` no daemon·start·bun 토큰/invalid throws).
> **독립 검증(내가 직접 재실행, main tree)**: `bun test ./apps/cli` **503 pass/0 fail** · `bun test ./packages/daemon` **466 pass/0 fail**(회귀 0) · `pnpm type-check:all` exit 0/0 error · `biome check apps/cli` clean. seam 코드 file:line 정독(`resolveDaemonSpawnCommand` override→`[path,[]]`·absent verbatim, catch 의 `stop()` spinner in-scope, `process.exit`=`never` 라 `resolved` definite-assign). **다음 = PR ship → dogfood refresh → 그다음 #4(default flip, E2E-gated) 또는 #5(Bun backend 삭제).**

> rev19 (2026-07-08): **daemon Rust 포팅 — 증분 4 머지(#895, main=`c85f9ddc`) + 증분 5(dispatcher+daemon root+shipping `[[bin]]`, XL) 코드+게이트 완료, PR 대기.**
> **inc4 머지**: push-gate differential 게이트가 CI(Linux)에서 실제 RUN 해 5개 required job GREEN, `gh api` PUT squash 머지(`c85f9ddc`).
> dogfood 재빌드+재설치 완료(tp v0.1.52, daemon=`tpd` 트램폴린 pid — running, brew symlink 무손상). **inc5 (branch `feat/tp-daemon-inc5-dispatcher`)**:
> 3개 모듈 포팅(correctness junction) — `ipc/command_dispatcher.rs`(~3189 LOC incl tests — `dispatch_ipc`+`dispatch_relay_control` 29 arms + 전 guard: stale-bye generation·`assert_safe_sid`·worktree `sanitize_for_sid`+orphan-rollback·worktree.remove live-session kill-on-force·`session.export` off-by-one·unsubscribe-on-delete·doctor.probe throttled),
> `daemon.rs`(~1596 — cyclic slot wiring, pairing poll-watcher, `start_auto_cleanup` panic-caught timer, stale-running sweep), `bin/tp_daemon.rs`(pid-lock→Daemon→auto-cleanup→reconnect→SIGINT/SIGTERM) + `[[bin]] tp-daemon`(dogfood 기본 아님). inc1-4 Store/SessionManager/IpcServer/WorktreeManager/RelayConnectionManager/PairingOrchestrator/PushNotifier verbatim 재사용.
> **게이트가 실제 파리티 버그 잡음 + 수정**: inc3 `to_wire_session_meta` 가 absent worktreePath/claudeVersion 을 explicit `null` 로 emit → TS `JSON.stringify` 는 undefined 키 drop → wire 생략(hello/state/export 프레임마다 발산). Rust omit-when-None 수정 + inc3 unit test 를 key-absence assert 로 강화.
> **게이트 = command-dispatcher Bun↔Rust differential** (`ipc/dispatcher-rust-parity.test.ts`, 4 pass/67 assert — **dispatcher-level**[정직: 양쪽 실 dispatcher in-process seam + 실 SQLite Store, IPC framing 은 ipc/server 별도 커버]: rank-3 traversal reject·rank-4 export off-by-one(markdown `d` byte-exact)·bye truth-table+stale-pid guard·sanitizeForSid 그리드; 신규 verbs `sanitize-sid`/`dispatch-bye`/`dispatch-create-sid`/`dispatch-export`, 미빌드 SKIP).
> **독립 검증(내가 직접 재실행 + null-vs-omit 수정이 `JSON.stringify` drop-undefined 와 일치함을 codec.ts:46 로 확인·promote-failure teardown 편차가 error-path-only 이고 observable end-state 동등함 확인·stale-bye guard+truth-table 정독): dispatcher 게이트 4 pass/67 assert · `cargo test -p tp-daemon` 184 pass(회귀 0, 16 daemon+32 dispatcher) · clippy 0 err · fmt clean · 양 bin(tp-daemon/tp-daemon-probe) 빌드 OK · Bun daemon 스위트 465 pass/2 skip(회귀 0).**
> **에이전트 flag(정직 disclose, 전부 검증됨)**: (#2) inc4 `save_pairing` 무결성 → `last_save_error` 슬롯 + promote-failure 시 `remove_pairing` teardown(TS 는 save-first-register-later 라 throw 시 client 미등록; Rust 는 등록 후 제거 → observable 동등), (#3) 완료 관찰 = 25ms poll-watcher(promise `.then` 대체, pairing_id 바인딩), (#4) completed-path pair 테스트는 orchestrator unit 이 커버(daemon 레벨 fake relay 가 wrapped events drop), (#5) `Store::close` consuming → drop-close·bin `--spawn` 실패는 log(TS top-level throw 편차, 주석). **게이트는 dispatcher-level(socket-level 아님) — 정직 disclose.** **다음 = inc6 (`TP_DAEMON_BIN` opt-in dual-run seam — apps/cli, M).**
> flip(별도 후속)·default 전환은 N× clean real-claude E2E 뒤. 상세 = plan 문서 §2 ladder.

> rev18 (2026-07-08): **daemon Rust 포팅 — 증분 3 머지(#894, main=`3963575d`) + 증분 4(pairing/push/relay-manager) 코드+게이트 완료, PR 대기.**
> **inc3 머지**: reconnect-policy differential 게이트가 CI(Linux)에서 실제 RUN 해 5개 required job GREEN, `gh api` PUT squash 머지(`3963575d`).
> dogfood 재빌드+재설치 완료(tp v0.1.52, daemon=`tpd` 트램폴린 — pairing 재연결+auth, 로그 clean, brew symlink 무손상). **inc4 (branch `feat/tp-daemon-inc4-orchestration`)**:
> 4개 모듈 포팅 — `transport/relay_manager.rs`(`RelayConnectionManager<D>`+`RelayManagerDeps` trait — pairing 당 1 `RelayClient` 풀, `build_events`,
> `remove_pairing` **store-first**, `rename_pairing`, `reconnect_saved`, `removing_daemon_ids` guard), `pairing/{pending_pairing,orchestrator,random_pairing_bundle}.rs`
> (single-slot `PendingPairing`, **rank-1 `on_frontend_joined` guard**(delegate 를 `tokio::spawn` 으로 decouple → signal record 무조건 실행),
> **genuinely-random `StaticSecret::random_from_rng(OsRng)` pairing keypair** — 결정론적 `kx_seed_keypair` 아님[libsodium `crypto_kx_keypair` 미러]),
> `push/notifier.rs`(`NOTIFY_EVENTS`+`tokenCount>0` gate, `build_push_message` — Notification title regex + code-point-safe truncation). crypto/msgs 는
> tp-core/tp-proto 재사용, store/session/relay-client 은 inc1/2/3 재사용. **게이트 = push-gate Bun↔Rust differential** (`push/push-notifier-rust-parity.test.ts`,
> 16 pass/31 assert — `interruptionLevelFor`/`buildPushMessage`+NOTIFY_EVENTS/tokenCount 게이트를 (eventName × tokenCount × payload) 그리드로 두 impl 에
> 돌려 shouldNotify/level/title/body byte-identical 검증[multibyte truncation 🇰🇷×200 포함]; 같은 `tp-daemon-probe` 의 신규 verb `push-gate` 로 driving, 미빌드 SKIP).
> **독립 검증(내가 직접 재실행 + store-first ordering·removingDaemonIds guard·buildEvents real-daemonId(빈 문자열 아님)·rank-1 guard·NOTIFY/token gate·
> random_pairing_bundle crypto[결정론적 kx_seed 거부 확인]·truncate multibyte-safety 정독): push-gate 파리티 16 pass/31 assert · `cargo test -p tp-daemon`
> 136 pass(회귀 0, 11 new relay_manager+notifier+pairing) · clippy 0 err · fmt clean · Bun daemon 스위트 무회귀.** **다음 = inc5 (dispatcher+daemon root+shipping `[[bin]]`, XL).**
