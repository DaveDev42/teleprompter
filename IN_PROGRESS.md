...
> - **#939 (`81a957b8`)**: `tp-relay` push ws-verdict TOCTOU 재검증 이식 (마지막 TS parity gap, 회귀 가드 5종).
> - **#940 (`eb1abd98`)**: rev23 #4 행이 "후속 sync" 로 미뤄둔 CLAUDE.md dogfood-freshness prose(fabler:guard 마커 커밋 `cc8e4a4e`)를 사용자 결정(유지)에 따라 clean 브랜치로 재조립해 머지 — #4 flip 문서 sync 완결.
> - **#941 (`c252c928`)**: **Xcode 27.0 beta(27A5228h)/macOS 27 beta 호스트 회귀 2건 대응** — (a) Metal Toolchain 이 별도 다운로드로 분리돼(`xcodebuild -downloadComponent MetalToolchain`) 미설치 시 SwiftTerm `Shaders.metal` 이 전 플랫폼 컴파일 실패 — 이 머신엔 설치 완료; (b) **windowless XCUITest launch 회귀**(앱은 부팅·활성인데 메인 창이 아예 생성 안 됨 — a11y 트리 Window 0개, `.onAppear` 미발화)를 `cmd_uitest` **호스트 게이트 #2** 로 SKIP 분류(3중 판별 + run 단위 fail-closed, `TP_UITEST_STRICT=1` escape hatch; 상세 = `.claude/rules/native-testing.md`).
> - **uitest-all 현 매트릭스** (이 호스트): ios PASS / ipad PASS / macos SKIP(`reason=windowless-launch`) / visionos PASS / watchos SKIP — exit 0, CI 7/7 green. **dogfood tp = v0.1.53 @ `c252c928` 재빌드+전체 adhoc 재서명 완료**, daemon `tp-daemon` running + `[RelayClient] authenticated to relay` 확인.
> - iPad/macOS 창 모델 분리 plan 파일(`~/.claude.elyvian/plans/scalable-munching-panda.md`)은 항목별 검증 결과 **이미 #907/#908/#909/#912 로 전부 출하 완료** — plan 파일은 stale (삭제 가능).
...
## 백엔드 Rust 포팅 flip 사다리 (✅ 완주 — 히스토리)

> 백엔드 Rust 포팅(daemon/runner/relay/cli)은 코드+파리티 게이트가 전부 main 에 있고, **#4 flip 머지 완료(#922 — default = Rust daemon+runner, 소크 5/5 clean 통과)**. 남은 건 **#5 Bun 삭제 캐스케이드** — PR 사다리로 진행 중: **PR1 #925(`tp run` native) → PR2a #926(`tp-loopback`) → PR3 #927(CI job body Rust 스왑) → PR4 #928(differential parity 게이트 삭제) → PR5 #929(daemon-side Bun 삭제) → PR2b #932(`rust/tp-e2e-holder` holder Rust 포트) 전부 머지**; **PR6 #933(TS 백엔드 소스+`tpd` blob 삭제) + PR7 #935(Node/Bun toolchain 제거 + version.txt SoT) 머지 완료**; **PR8 #936(docs prose 정리) 머지 완료 — 캐스케이드 전체(PR1–PR8) 종료, #5 ✅**. 태스크는 4개로 통합 (구 #23→#17, 구 #8→#4, 구 #6/#7→#5 흡수). 의존: **#17 + #18 + #4 → #5**.
