# IN_PROGRESS — 후속 세션 인계 (2026-07-06 갱신 · rev10)
...
## 상태 스냅샷 (origin/main = 65f5958f, 2026-07-06 rev10)
...
> green, workspace clippy/fmt clean, 무회귀. 상세 = "즉시 다음 액션" 항목 9. **다음 = 증분 2(tokio async 오케스트레이션).**
>
...
9. 🔨 **runner Rust 포팅 착수 — 증분 1 (rev10, 이 세션)** — 사용자 지시("runner 포팅 착수")로 ADR-0003
   Stage 4 시작. `rust/tp-runner` 크레이트 스캐폴딩(workspace member 추가) + 순수·테스트가능 조각:
   **byte-exact `capture_hook_command`**(golden 테스트 — 셸 실행되는 `bun -e` 문자열이라 바이트 동일 필수) +
   `build_settings`(16 HOOK_EVENTS hook 머지, 기존 settings 보존) + `collector`(io = 바이너리 사이드카
   `payload=""` = Stage 4 parity gate / event = base64) + **portable-pty PTY spike**(spawn/read/write/resize/
   kill, reader-thread hop, echo/cat 라운드트립 통과). **ADR §6.1 "단일 최대 기술 미지수"(PTY 크레이트) 해소** —
   portable-pty 확정. tp-runner 12 테스트 green · workspace clippy(`all=deny`)·fmt clean · 전 워크스페이스
   test 무회귀. `main.rs` 는 아직 프로덕션 미결선(비-제로 exit — dual-run seam `TP_RUNNER_BIN` 미cutover).
   **다음(증분 2)**: tokio IPC 클라이언트(decode-throw teardown) + hook receiver(sid traversal 가드, UTF-8 cap) +
   `Runner` 상태머신(pid 생성 가드, graceful bye-flush) + main.rs argv + daemon `TP_RUNNER_BIN` seam 단일세션 dual-run.
...
> - ⬜ **runner** (`packages/runner`, 763 LOC, PTY + hooks) — **미포팅, 다음 증분** (ADR 순서: runner → daemon).
> - ⬜ **daemon** (`packages/daemon`, 5,391 LOC) — 미포팅, 최대·최종 조각 (store rusqlite 포맷 동결 / worktree
>   git2-vs-shell / PTY 크레이트 spike = ADR §6 열린 결정).
>
> **⇒ 다음 코딩 작업 = runner Rust 포팅 (그 다음 daemon).** ADR-0003 Accepted 이라 새 승인 불요 —
> 착수 여부(투자 시점)만 사용자 결정. PTY 크레이트 선택 등 ADR §6 spike 를 포팅 전에 정한다.
>
