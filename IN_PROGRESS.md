...
**현재 브랜치**: `chore/pr7-remove-node-toolchain` (base = `b4be4742` = PR6 #933 squash). #4 flip 머지 완료(#922) — dogfood daemon 은 Rust `tp-daemon` 으로 구동, dogfood tp 는 PR6 반영 재빌드 완료.
...
> 백엔드 Rust 포팅(daemon/runner/relay/cli)은 코드+파리티 게이트가 전부 main 에 있고, **#4 flip 머지 완료(#922 — default = Rust daemon+runner, 소크 5/5 clean 통과)**. 남은 건 **#5 Bun 삭제 캐스케이드** — PR 사다리로 진행 중: **PR1 #925(`tp run` native) → PR2a #926(`tp-loopback`) → PR3 #927(CI job body Rust 스왑) → PR4 #928(differential parity 게이트 삭제) → PR5 #929(daemon-side Bun 삭제) → PR2b #932(`rust/tp-e2e-holder` holder Rust 포트) 전부 머지**; **PR6(TS 백엔드 소스+`tpd` blob 삭제) 진행 중 — 브랜치 `feat/pr6-delete-ts-backend`, 소스 삭제+리와이어+문서 sync 완료, 검증/ship 대기**; 잔여 = PR7(Node/Bun toolchain 제거) → PR8(docs prose 정리). 태스크는 4개로 통합 (구 #23→#17, 구 #8→#4, 구 #6/#7→#5 흡수). 의존: **#17 + #18 + #4 → #5**.
