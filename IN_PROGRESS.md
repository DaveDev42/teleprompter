# IN_PROGRESS — 후속 세션 인계 (2026-07-06 갱신 · rev8)
...
## 상태 스냅샷 (origin/main = 65f5958f, 2026-07-06 rev8)

> rev8 (2026-07-06, 검증 전용 세션): **미착수 코딩 백로그 없음 — 실제 상태를 문서가 아니라 git/테스트로 재확인.**
...
> **⛔ 미착수 코딩 백로그 없음.** #44–#51 태스크 보드 완전 종료. 남은 열린 항목은 **전부 사용자 결정 /
> 배포 액션 / 업스트림 게이트** — 에이전트가 자율 진행할 수 없는 것들이다. 다음 세션이 이어가려면
> 사용자가 아래 중 하나를 명시적으로 언블록해야 한다:
>
> - **Phase 4 — 백엔드 Rust 이관** (TODO.md:84, ADR-0003 *Proposed*): "Dave 승인 대기, 승인 전 코드
>   cutover 금지". 첫 증분은 Stage 0(메시지 골든벡터)+Stage 1(relay) 한정. **결정 필요 9건**(PTY 크레이트
>   spike, CLI 이동 시점, `--compile`→cargo, git2-vs-shell, rusqlite 포맷 동결, hook 헬퍼, 크레이트 확정,
>   시크릿 공유, 10k soak 소유자 = ADR §6). → 승인 시 Stage 0+1 만 착수.
> - **터미널 렌더러 최종 결정** (TODO.md:101): libghostty(불안정 C API) vs SwiftTerm(non-Metal) — "Dave
>   결정 필요". *참고: Phase 3 는 이미 SwiftTerm 인터랙티브로 출하됨 — 이 항목은 렌더러 교체 여부 재검토 성격.*
> - **External TestFlight (Beta App Review)** (TODO.md:93): internal 은 완료, external 테스터 확장은 첫 빌드
>   Beta App Review 제출 필요(`externalBuildState=READY_FOR_BETA_SUBMISSION`). 배포 액션 — 사용자 go-ahead 필요.
> - **Claude Code channels 양방향** (TODO.md:97): 업스트림(Claude Code output 구독) 지원 시 Chat UI 통합
>   재검토 — 업스트림 기능 게이트, 현재 불가.
>
