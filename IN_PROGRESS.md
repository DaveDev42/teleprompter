...
## 상태 스냅샷 (origin/main = 74325a80, 2026-07-05 rev4)

> rev4: **PR-6 (Option A synced pairing store) 구현·검증 완료, PR 준비** — 8-PR 페어링 재설계 중
...
> 남은 것: PR-7(unpair/local-hide split)·PR-8(`WS_PROTOCOL_VERSION` 2→3 + 문서).
>
...
| #49 | 🔄 진행 | Pairing 재설계 8-PR — **PR-1~PR-5 머지 완료 (#863/#864/#867/#869/#871), PR-6(Option A synced pairing store) 구현·검증 완료 PR 준비, PR-7·PR-8 미착수** — 아래 상세 |
...
**PR-6 (Option A synced pairing store) — 구현·검증 완료 (PR 준비)**:
...
**남은 PR (설계 §5)**:
- **PR-7**: unpair vs "이 기기에서만 제거" split (localHidden tombstone, pairingId 키). **오프라인-miss
  된 peer unpair 의 negative-evidence tombstone 을 소유** (PR-6 는 부재를 unpair 증거로 삼지 않음).
...
4. ✅ **#49 PR-6 (Option A synced pairing store) — 구현·검증 완료, PR 준비** (이 세션, 사용자
   greenlight 후). `PairingRecordStore` seam + synced whole-record blob + 레거시 마이그레이션 +
   macOS 런타임 프로브 + 포인터 reconciliation. 적대적 리뷰 14 findings 반영(save-before-sweep,
   부분-sync 포인터 보존, orphan sweep, retry-on-unlock). XCTest 178/178 · iOS smoke 8/8 ×2 ·
   macOS 8/8. §3.7 2-device iCloud = 수동 게이트(CI 불가). **dogfood 재빌드 불필요(app-only).**
5. ⏭️ **#49 PR-7 (다음) — unpair vs "이 기기에서만 제거" split** (localHidden tombstone, pairingId 키).
   PR-6 가 남긴 오프라인-miss peer-unpair 의 negative-evidence tombstone 을 소유. 그 뒤
   PR-8(`WS_PROTOCOL_VERSION` 2→3 + PCT/v4 문서 갱신).
