...
## 상태 스냅샷 (origin/main = 12594403, 2026-07-05 rev5)

> rev5: **PR-7 (unpair vs "이 기기에서만 제거" split) 구현·검증 완료, PR 준비** — 8-PR 페어링 재설계 중
...
| #49 | 🔄 진행 | Pairing 재설계 8-PR — **PR-1~PR-6 머지 완료 (#863/#864/#867/#869/#871/#873), PR-7(unpair/local-hide split) 구현·검증 완료 PR 준비, PR-8 미착수** — 아래 상세 |
...
**PR-7 (unpair vs "이 기기에서만 제거" split) — 구현·검증 완료 (PR 준비)**:
...
5. ✅ **#49 PR-7 (unpair vs "이 기기에서만 제거" split) — 구현·검증 완료, PR 준비** (이 세션).
   device-local·NON-synced `localHidden` tombstone. `hideLocally` non-revoking(blob/secret 미삭제,
   `control.unpair` 미발신). reconcile hidden-필터를 loser-sweep 앞단(HIGH). legacy 결정론 →
   recommit 이 legacy-derived id 도 unhide. sidecar 보존 + PENDING sweep. 2-버튼 confirm 시트.
   적대적 설계 리뷰 28 agents/17 confirmed. XCTest 187/187 · iOS/macOS smoke 8/8. **app-only.**
   그 뒤 PR-8(`WS_PROTOCOL_VERSION` 2→3 + PCT/v4 문서 갱신).
6. **#50, crash 로그(#45)** — 사용자 액션 대기 (리마인드만; 대신 실행 금지).
