...
- **PR-4 (앱 Swift connect-on-pending 라이프사이클) — 브랜치 `feat/pr4-swift-pending-lifecycle`
  구현+검증 완료, PR 착지 진행 중**. PENDING 네임스페이스(device-local, non-synced Keychain) + ingest→
  `TP_PAIR_PENDING` + `beginPending` connect-on-pending + kx 완료 시 promote(살아있는 client 를
  재연결 없이 committed 맵으로 re-key, §1.6 R2) + `TP_PAIR_OK` 를 promote-time 으로 이동 + committed
  meta `pairingId`/`hostname` 영속 + 레거시 `deriveLegacyPairingId` backfill + QR v4 필드. 8 core
  파일 + watch companion(`WatchPairingViewModel`) + 하니스 M1 mode-split(real-E2E=`TP_PAIR_PENDING`,
  loopback=`TP_PAIR_OK`). **검증**: XCTest 157/157 (iOS Sim), macOS loopback smoke 8/8 (M1 이
  promote-time `TP_PAIR_OK` 로 정상 발화), swift-format lint clean(변경 11 파일). **PCT 검증은 PR-5**
  — PR-4 승격 조건은 kx 완료(레거시 의미론)라 독립 착지.
- **⏳ dogfood tp 재빌드 미완** — #867 이 `packages/{daemon,protocol}/**` 를 건드렸으므로
  CLAUDE.md freshness 룰상 재빌드 필요. PR-4 는 `ios/**`+`scripts/ios.sh` 만 건드려(백엔드 무변경)
  dogfood tp 영향 없음 — #867 재빌드만 하니스/worktree idle 시 실행.

**현재 브랜치**: `feat/pr4-swift-pending-lifecycle`. PR-3 worktree 들은 prune 완료.
...
| #49 | 🔄 진행 | Pairing 재설계 8-PR — **PR-1/PR-2/PR-3 머지 완료 (#863/#864/#867), PR-4 착지 진행(앱 connect-on-pending), PR-5~PR-8 미착수** — 아래 상세 |
...
1. **dogfood tp 재빌드** — #867 이 daemon/protocol 을 건드림. visionOS E2E 하니스 종료 확인 후
   CLAUDE.md freshness 시퀀스(Rust tp + tpd blob + adhoc 재서명 + `daemon install`) 실행.
2. **#49 PR-4 (앱 Swift PCT 배선) 착수** — PR-1/2/3 머지됨. app pending 라이프사이클 + 3 ingest 지점 +
   committed meta pairingId/hostname + `TP_PAIR_PENDING` 마커 + real-E2E M1 재앵커. 설계 §5 PR-4 행.
   **PCT 검증은 PR-5** — PR-4 승격 조건은 kx 완료(레거시 의미론)로 독립 착지.
