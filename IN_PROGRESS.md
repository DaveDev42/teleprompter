...
**현재 브랜치**: `main` (= `3325b4c0`, #5 캐스케이드 PR5 = 데몬-사이드 Bun 삭제 #929 반영). #4 flip 머지 완료(#922) — dogfood daemon 은 Rust `tp-daemon` 으로 구동.

worktree 상태: 메인 worktree 하나만. stash 없음, 로컬 브랜치 `main` 뿐.

---

## 라이브 로드맵 — 백엔드 Rust 포팅 flip 사다리 (미완, 의존 체인)

> 백엔드 Rust 포팅(daemon/runner/relay/cli)은 코드+파리티 게이트가 전부 main 에 있고, **#4 flip 머지 완료(#922 — default = Rust daemon+runner, 소크 5/5 clean 통과)**. 남은 건 **#5 Bun 삭제 캐스케이드** — PR 사다리로 진행 중: **PR1 #925(`tp run` native) → PR2a #926(`tp-loopback`) → PR3 #927(CI job body Rust 스왑) → PR4 #928(differential parity 게이트 삭제) → PR5 #929(daemon-side Bun 삭제) 전부 머지**; **PR2b(`rust/tp-e2e-holder` — real-claude E2E holder Rust 포트, PR6 의 hard blocker) 진행 중**; 잔여 = PR6(TS 백엔드 소스+`tpd` blob 삭제) → PR7(Node/Bun toolchain 제거) → PR8(docs prose 정리). 태스크는 4개로 통합 (구 #23→#17, 구 #8→#4, 구 #6/#7→#5 흡수). 의존: **#17 + #18 + #4 → #5**.
...
| #5 | 🔨 진행 중 | **Bun 삭제 캐스케이드(구 #6/#7 흡수)** — PR 사다리: **PR1 #925**(`tp run` native, Route::Forward 퇴역) / **PR2a #926**(Rust `tp-loopback` smoke 백엔드) / **PR3 #927**(CI 5 job body Rust 스왑, context 이름 보존) / **PR4 #928**(6 differential parity 게이트 + `tp-daemon-probe` 삭제) / **PR5 #929**(daemon-side Bun 삭제) **머지 완료**. **PR2b**(`rust/tp-e2e-holder` — real-claude E2E holder 를 Bun `real-daemon-pair.ts` 에서 Rust 로 포트, `scripts/ios.sh` rewire 포함; **PR6 hard blocker**) 검증 매트릭스 완주(REAL/CLAUDE/M5/CODING+양 parity/WEBPAGE/PUSH 전부 macOS PASS) — **PUSH 룽이 pre-existing 실버그 2건을 적발**(둘 다 스택): (1) 앱 합성 push 토큰이 #839 의 64-hex APNs wire guard 에 걸려 silent drop → `RelayClient.swift` frontendId-유도 64-hex 로 수정, (2) Rust `tp-relay` `handle_push` 가 APNs 미설정(`push_service==None`) 시 **in-band `relay.notification` leg 까지** early-return drop(TS 파리티 위반, push.ts step 1 "ws" 는 APNs 불요) → None arm 에서 in-band 직접 전달 + 회귀 가드 2건(`conn.rs`) 로 수정. 소크+iOS spot-check 후 pr-shipper(DO NOT MERGE) 예정. 잔여: **PR6** Stage 1 backend 소스 삭제(`packages/*`+`apps/cli`+`tpd` blob; 직전에 #18 `message-vectors.json` regen) → **PR7** Stage 2 Node/Bun toolchain 제거(Turborepo/pnpm/Biome-TS/lefthook 재배선) → **PR8** Stage 3 문서 stale prose 정리 | #17/#18/#4 ✅ · PR2b → PR6 → PR7 → PR8 |
