...
| #4 | 🔨 구현완료·soak 대기 | **default flip 통합(구 #8 흡수)**: daemon→`tp-daemon` + runner→`tp-runner`. ship+locate 완료(#898/A2, #906). **flip 배선 완료**: (a) runner — `manager.rs` `default_runner_command` 가 `tp_proto::locate_tp_runner()` 로 Rust `tp-runner` spawn(`["bun","run"]` placeholder 제거), resolver 를 `tp-proto::locate` 로 이동(daemon+cli 공유 SoT, 순환의존 회피); (b) daemon — `ensure_daemon.rs` background auto-spawn + `commands/daemon.rs::start` foreground/OS-service trampoline 둘 다 `locate_tp_daemon()` 로 Rust `tp-daemon` exec(총 flip). Rust 전체 green(tp-proto 57+5 / tp-cli 318 / tp-daemon 186, +2 flip 테스트), bun 961 / type-check 0. **homebrew-tap formula 수정 = 사용자 확인 후 push**(tp-daemon/tp-relay/tp-runner를 `def install` 에 추가, `if File.exist?` 가드). CLAUDE.md dogfood-freshness prose(pgrep tpd→tp-daemon, prefix-tree) 는 fabler:guard 로 이번 커밋서 제외 — 후속 sync. **소크 커버리지 갭**: soak holder 는 runner 를 standalone spawn 하지 daemon 의 `SessionManager.spawn_runner`(manager.rs 새 경로)를 경유 안 함 → 그 경로는 dogfood+수동 커버 | **N× clean 실 claude E2E soak(사용자 구동, 5 macOS-native: 3×M5 + CODING + WEBPAGE, 각 `TP_E2E_DAEMON_BIN=1 TP_E2E_RUNNER_BIN=1`) → 통과 후 merge** |
...
2. 🔨 **#4 default flip (daemon+runner, 구 #8 흡수) — 구현 완료, soak-merge 대기** — ship+locate 완료(#898/#906),
   **flip 배선 완료**: runner(`manager.rs`→`tp_proto::locate_tp_runner`) + daemon(`ensure_daemon.rs` background +
   `commands/daemon.rs::start` foreground/service → `locate_tp_daemon`). Rust/bun/type-check 전부 green. 실 claude
   E2E soak(5 macOS-native, 사용자 구동) 통과 후 merge. #18(`message-vectors.json` 만 재생성, 비가역)은 #5 Stage 1 직전 — 사전검증 완료(idempotent), 위 #18 상세 참조.
