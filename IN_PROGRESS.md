...
> **✅ 백엔드 Rust 포팅 사다리 — 완주 (ADR-0003 Phase 4, rev15~22, 전부 main 머지).** daemon(7,014 LOC)+runner 를 `rust/tp-{daemon,runner,relay,cli}` 로 leaf-first 포팅. **flip 완료 (#4/#922 머지, daemon+runner 통합) — 로컬 N× clean 실 claude E2E soak(5 macOS-native) 통과 후 머지됨.** Rust 데몬이 `locate_tp_runner()` 로 Rust runner spawn, Rust CLI 의 background+foreground/service daemon 경로가 `locate_tp_daemon()` 로 Rust daemon exec. 각 증분의 상세 SoT = 각 PR + `docs/design/daemon-rust-port-plan.md` §2 ladder + ADR-0003. 증분별 CI-결정론 differential 파리티 게이트(store/worktree/relay-client/push/dispatcher/runner) = `.claude/rules/ci-workflows.md`. 핵심 마일스톤:
> - **runner** inc1~4 머지(#882/#883/#884/#890) — 스캐폴드+portable-pty spike → tokio async 오케스트레이션 → `TP_RUNNER_BIN` dual-run seam → real-claude parity 증명 게이트. **relay flip 머지(#905)** 로 `run` 이 blob 유일 route(`Route::Forward`). **runner flip-prep 머지(#906=d45ed353)**: `tp-runner` release 아티팩트 ship + `locate_tp_runner()`. **#4 flip**: resolver 를 `tp-proto::locate` 로 이동(daemon+cli 공유 SoT), `manager.rs default_runner_command` 가 첫 caller — Rust 데몬이 Rust `tp-runner` 를 기본 spawn.
...
> - **de-trampoline tp-cli(#17, ✅ 완료)**: 코어 3종(#903 native passthrough / #904 util forwards+`tp --` / #905 relay redirect) + soft couplings(`locate.rs` sentinel→`rust/tp-cli/Cargo.toml`, `upgrade.rs` pgrep `tpd`+`tp-daemon`) + **PR-5(native first-run install 프롬프트 #917 + apps/cli passthrough 삭제·index.ts 리와이어, 구 #23 흡수)** 완료. Part B 게이트 = 로컬 실 claude 두 증거체(네이티브 passthrough 직접 5/5 + substrate soak 5/5). **#4 flip 머지 완료(#922) → 현재 #5(Bun 삭제 캐스케이드) 진행 중 — 아래 라이브 로드맵 참조.**

**현재 브랜치**: `chore/pr8-docs-prose-purge` (base = `3f5b89cc` = PR7 #935 squash). #4 flip 머지 완료(#922) — dogfood daemon 은 Rust `tp-daemon` 으로 구동, dogfood tp 는 PR7 반영 재빌드 완료(v0.1.53, version.txt SoT 경로 검증).
...
> 백엔드 Rust 포팅(daemon/runner/relay/cli)은 코드+파리티 게이트가 전부 main 에 있고, **#4 flip 머지 완료(#922 — default = Rust daemon+runner, 소크 5/5 clean 통과)**. 남은 건 **#5 Bun 삭제 캐스케이드** — PR 사다리로 진행 중: **PR1 #925(`tp run` native) → PR2a #926(`tp-loopback`) → PR3 #927(CI job body Rust 스왑) → PR4 #928(differential parity 게이트 삭제) → PR5 #929(daemon-side Bun 삭제) → PR2b #932(`rust/tp-e2e-holder` holder Rust 포트) 전부 머지**; **PR6 #933(TS 백엔드 소스+`tpd` blob 삭제) + PR7 #935(Node/Bun toolchain 제거 + version.txt SoT) 머지 완료**; 잔여 = PR8(docs prose 정리 — 진행 중, 브랜치 `chore/pr8-docs-prose-purge`). 태스크는 4개로 통합 (구 #23→#17, 구 #8→#4, 구 #6/#7→#5 흡수). 의존: **#17 + #18 + #4 → #5**.

| # | 상태 | 요약 | 게이트/의존 |
|---|---|---|---|
| #17 | ✅ 완료 | de-trampoline tp-cli 완료 — 코어 3종(relay #905 / util forwards+`tp --` #904 / native passthrough #903) + soft couplings(`locate.rs` repo-root sentinel → `rust/tp-cli/Cargo.toml`, `upgrade.rs` pgrep `tpd`+`tp-daemon` 병행 매칭) 머지 완료. **PR-5 Part A(native first-run install 프롬프트)** — `showInstallHint` 를 `rust/tp-cli/src/ensure_daemon.rs` 로 포팅(#917, `5e5b113e` 이후 별도 머지). **PR-5 Part B(apps/cli passthrough 삭제, 구 #23 흡수) 완료** — `apps/cli/src/commands/passthrough.ts`+`passthrough.test.ts` 삭제, Bun `index.ts` `case "passthrough"` 를 `forwardToClaudeCommand`(daemon-bypass claude 포워드, `tp -- <args>` 와 동일)로 리와이어(Bun CLI 를 #5 전까지 동작 레퍼런스로 유지). 2번째 gap `showFirstRunPairing`(welcome+pair+install, `listPairings==0` 게이트)은 native passthrough 자체 first-run 으로 별도 포팅(ensure_daemon 모듈 doc 기록, out-of-scope) | Part A = 3-lens adversarial 0 confirmed. **Part B 게이트 = 두 증거체(로컬 실 claude, 절대 CI 아님)**: (1) 네이티브 passthrough 직접 exercise 5/5 clean (`rust/target/release/tp -p` → Route::Passthrough, 격리 XDG, Stop≥1 DB proof) — 매핑 워크플로 wbv9n3xu0 이 soak 만으로는 passthrough.rs 를 안 탄다(orthogonal)고 증명해 추가. (2) substrate soak 5/5 (3× M5 + CODING + WEBPAGE). #5 앞단 게이트 통과 |
| #18 | ✅ 완료 (PR6 에서 소비) | `packages/protocol` 삭제 직전 **`message-vectors.json` 만** 재생성 — PR6 브랜치에서 최종 regen 실행, **byte-identical(zero-diff)** 재확인 후 protocol 삭제 진행. `wire-vectors.json` 은 자동생성 대상 아님 = frozen KAT, 그대로 생존 — 아래 상세 | #5 앞단 ✅ |
| #4 | ✅ 완료 (#922 머지) | **default flip 통합(구 #8 흡수)**: daemon→`tp-daemon` + runner→`tp-runner`. ship+locate 완료(#898/A2, #906). **flip 배선 완료 (PR #922, `db83b32a`)**: (a) runner — `manager.rs` `default_runner_command` 가 `tp_proto::locate_tp_runner()` 로 Rust `tp-runner` spawn(`["bun","run"]` placeholder 제거), resolver 를 `tp-proto::locate` 로 이동(daemon+cli 공유 SoT, 순환의존 회피); (b) daemon — `ensure_daemon.rs` background auto-spawn + `commands/daemon.rs::start` foreground/OS-service trampoline 둘 다 `locate_tp_daemon()` 로 Rust `tp-daemon` exec(총 flip). Rust 전체 green(tp-proto 57+5 / tp-cli 318 / tp-daemon 186, +2 flip 테스트), bun 961 / type-check 0. CI 7/7 green(lint/type-check/test/build-cli/rust/swift-build/swift-smoke-ios). **dogfood dry-run**: `tp daemon start` 가 `tp-daemon` 으로 exec + 소켓 bind + clean SIGTERM, exit-137 없음(plain cargo bin 은 codesign 불필요 — tpd 블롭만 필요). **homebrew-tap formula push 완료**(DaveDev42/homebrew-tap `17c3e422` — tp-daemon/tp-relay/tp-runner를 `def install` 에 `if File.exist?` 가드로 추가, awk-target 필드 불변). CLAUDE.md dogfood-freshness prose(pgrep tpd→tp-daemon, prefix-tree) 는 fabler:guard 로 이번 커밋서 제외 — 후속 sync. **소크 커버리지 갭**: soak holder 는 runner 를 standalone spawn 하지 daemon 의 `SessionManager.spawn_runner`(manager.rs 새 경로)를 경유 안 함 → 그 경로는 dogfood+수동 커버 | N× clean 실 claude E2E soak(5 macOS-native: 3×M5 + CODING + WEBPAGE) **통과 → #922 머지 완료** |
| #5 | 🔨 진행 중 | **Bun 삭제 캐스케이드(구 #6/#7 흡수)** — PR 사다리: **PR1 #925**(`tp run` native, Route::Forward 퇴역) / **PR2a #926**(Rust `tp-loopback` smoke 백엔드) / **PR3 #927**(CI 5 job body Rust 스왑, context 이름 보존) / **PR4 #928**(6 differential parity 게이트 + `tp-daemon-probe` 삭제) / **PR5 #929**(daemon-side Bun 삭제) **머지 완료**. **PR2b #932**(`rust/tp-e2e-holder` — real-claude E2E holder Rust 포트, `scripts/ios.sh` rewire 포함) **머지 완료** — 검증 매트릭스 완주(REAL/CLAUDE/M5/CODING+양 parity/WEBPAGE/PUSH 전부 macOS PASS), PUSH 룽이 pre-existing 실버그 2건(앱 합성 push 토큰 64-hex guard drop → `RelayClient.swift` 수정, `tp-relay` `handle_push` APNs-미설정 시 in-band leg drop → None-arm 직접 전달 + 회귀 가드) 적발·수정 포함. **PR6 #933 머지 완료**: backend TS 소스 삭제(`packages/*`+`apps/cli`+root tsconfig+dev-harness .ts, ~298 paths) + `tpd` blob 퇴역(릴리즈 tarball 은 pre-PR6 호환 sh 스텁만) + stay-green 리와이어 + **#18 final regen zero-diff 확정**(sha256 `3effc9a9…` 불변) — 7/7 CI green(리와이어된 swift-smoke-ios 포함), 29-agent adversarial 검증 7건 수정 후 ship. **PR7 #935 머지 완료** (`chore/pr7-remove-node-toolchain`): Node/Bun toolchain 완전 제거 — root `package.json`/`pnpm-lock.yaml`/`pnpm-workspace.yaml`/`turbo.json`/`biome.json`/`.npmrc`/`bunfig.toml`/`.node-version` 삭제, **버전 SoT 를 `version.txt` 로 마이그레이션**(release-please `release-type: node→simple`, `tp-cli/build.rs` 가 version.txt 읽음 — `tp v0.1.53` 검증), `lefthook.yml` biome 블록 제거(rust pre-push 만 유지, brew 설치 경로), dependabot npm 블록 제거(+무효화된 npm PR #911/#934 close), `doctor.rs` Node/pnpm legacy probe 삭제, `.gitignore` Node/Expo 잔재 정리, cw post-create pnpm hook 제거, docs/rules sync — 7/7 CI green, 21-agent adversarial 검증 4건 수정 후 ship. **PR8 진행 중** (`chore/pr8-docs-prose-purge`): Stage 3 문서 stale prose 정리 — dead-pathed rules 2종 삭제(backend-services.md/testing-inventory.md — paths frontmatter 가 삭제된 `packages/**`/`apps/cli/**` 를 가리켜 더는 auto-load 안 됨), `protocol.md`/`relay-capacity.md` 전면 Rust-era 재작성(PR6 bridge banner 제거, TS 인용 → 라이브 Rust file:line), rust 소스의 삭제-rule 참조 주석 2곳 self-contained 화 | #17/#18/#4 ✅ · PR6 ✅ · PR7 ✅ → PR8 |
...
- ✅ first-run 프롬프트 자동 응답이 3개 holder 모드(interactive/coding/webpage) 전부에 배선됨 —
  **#5 PR2b 이후 holder = Rust `tp-e2e-holder`** (`rust/tp-e2e-holder/src/claude.rs`
  `answer_first_run_prompts`, 구 TS `answerFirstRunPrompts` 1:1 포트). ⚠️ 이 로직은
  `scripts/ios.sh` 에는 없다 — ios.sh 는 bash 오케스트레이션 하니스이고, holder 가 세션을 소유하고
  claude 턴을 진행한다. (`scripts/real-daemon-pair.ts` 는 `rust-relay-e2e.ts` 의존 때문에
  PR6 까지 dead 상태로 tree 에 남는다.)
...
1. ✅ **#17 de-trampoline tp-cli 완료** — 코어 3종(#903/#904/#905) + soft couplings(`locate.rs`
   sentinel→`rust/tp-cli/Cargo.toml`, `upgrade.rs` pgrep `tpd`+`tp-daemon`) 머지 완료. **PR-5 Part A
   (native first-run install 프롬프트 = `showInstallHint` → `ensure_daemon.rs`, #917)** + **Part B
   (`apps/cli` passthrough 삭제 + `index.ts` → `forwardToClaudeCommand` 리와이어, 구 #23 흡수)** 완료.
   Part B 게이트 = 로컬 실 claude 두 증거체(네이티브 passthrough 직접 5/5 clean + substrate soak 5/5:
   3× M5 + CODING + WEBPAGE) — 매핑 워크플로가 soak 만으로는 passthrough.rs 를 안 탄다(orthogonal)고
   증명해 직접 exercise 를 추가했다.
2. 🔨 **#4 default flip (daemon+runner, 구 #8 흡수) — PR #922 open, CI 7/7 green, soak-merge 대기** — ship+locate 완료(#898/#906),
   **flip 배선 완료**(`db83b32a`): runner(`manager.rs`→`tp_proto::locate_tp_runner`) + daemon(`ensure_daemon.rs` background +
   `commands/daemon.rs::start` foreground/service → `locate_tp_daemon`). Rust/bun/type-check 전부 green, dogfood dry-run
   exit-137 없음, homebrew-tap push 완료(`17c3e422`). 실 claude E2E soak(5 macOS-native, 사용자 구동) 통과 후 #922 merge.
   #18(`message-vectors.json` 만 재생성, 비가역)은 #5 Stage 1 직전 — 사전검증 완료(idempotent), 위 #18 상세 참조.
