...
> - **de-trampoline tp-cli(#17, 진행 중)**: 코어 3종(#903 native passthrough / #904 util forwards+`tp --` / #905 relay redirect) 머지 완료. **다음 = soft couplings + PR-5(native first-run + apps/cli passthrough 삭제, 구 #23 흡수, N× clean 실 claude soak-gated) → #4 flip(daemon+runner) → #5(Bun 삭제 캐스케이드).**
...
| # | 상태 | 요약 | 게이트/의존 |
|---|---|---|---|
| #17 | 🔨 진행 중 | de-trampoline tp-cli 마무리 — 코어 3종(relay #905 / util forwards+`tp --` #904 / native passthrough #903) 머지 완료. 잔여 = (a) soft couplings(`locate.rs` repo-root sentinel `apps/cli/src/index.ts`→내구 sentinel, `upgrade.rs` pgrep `tpd`→`tp-daemon` 병행 매칭), (b) PR-5(구 #23): native first-run 프롬프트 + `apps/cli` passthrough 삭제 | PR-5 는 N× clean 로컬 실 claude soak(사용자 구동); #5 앞단 게이트 |
...
1. 🔨 **#17 de-trampoline tp-cli 마무리** — 코어 3종(#903/#904/#905) 머지 완료. 잔여 = soft couplings
   (`locate.rs` sentinel, `upgrade.rs` pgrep) + PR-5(native first-run 프롬프트 + `apps/cli` passthrough 삭제,
   구 #23 흡수 — N× clean 로컬 실 claude soak 게이트, 사용자 구동).
