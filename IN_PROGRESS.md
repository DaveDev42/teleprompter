...
**현재 브랜치**: `feat/pr6-delete-ts-backend` (base = `848f03a6`, #5 캐스케이드 PR2b = Rust `tp-e2e-holder` #932 머지 반영). #4 flip 머지 완료(#922) — dogfood daemon 은 Rust `tp-daemon` 으로 구동.
...
| #5 | 🔨 진행 중 | **Bun 삭제 캐스케이드(구 #6/#7 흡수)** — PR 사다리: **PR1 #925**(`tp run` native, Route::Forward 퇴역) / **PR2a #926**(Rust `tp-loopback` smoke 백엔드) / **PR3 #927**(CI 5 job body Rust 스왑, context 이름 보존) / **PR4 #928**(6 differential parity 게이트 + `tp-daemon-probe` 삭제) / **PR5 #929**(daemon-side Bun 삭제) **머지 완료**. **PR2b #932**(`rust/tp-e2e-holder` — real-claude E2E holder Rust 포트, `scripts/ios.sh` rewire 포함) **머지 완료** — 검증 매트릭스 완주(REAL/CLAUDE/M5/CODING+양 parity/WEBPAGE/PUSH 전부 macOS PASS), PUSH 룽이 pre-existing 실버그 2건(앱 합성 push 토큰 64-hex guard drop → `RelayClient.swift` 수정, `tp-relay` `handle_push` APNs-미설정 시 in-band leg drop → None-arm 직접 전달 + 회귀 가드) 적발·수정 포함. **PR6 진행 중** (`feat/pr6-delete-ts-backend`): backend TS 소스 삭제(`packages/*`+`apps/cli`+root tsconfig+dev-harness .ts, 261 파일) + `tpd` blob 퇴역(릴리즈 tarball 은 pre-PR6 호환 sh 스텁만) + stay-green 리와이어(`Route::Forward`/`exec_blob`/`locate_bun_blob` 삭제, `upgrade.rs` 번들 일반화, `release.yml` Rust 번들(`scripts/build-bundle.sh`), CI/CODEOWNERS/slash-commands/rules 문서 sync) + **#18 final regen zero-diff 확정**(sha256 `3effc9a9…` 불변). cargo 전체 green(fmt/clippy/26 suites). 잔여 = 검증 pass(f)+adversarial(g)+pr-shipper ship(DO NOT MERGE, h). 그 뒤: **PR7** Stage 2 Node/Bun toolchain 제거(Turborepo/pnpm/Biome-TS/lefthook 재배선) → **PR8** Stage 3 문서 stale prose 정리(backend-services.md/testing-inventory.md 삭제, protocol.md 본문 재작성) | #17/#18/#4 ✅ · PR6 → PR7 → PR8 |
