...
5.1 컴포넌트
	•	Frontend(앱): Swift(SwiftUI) iOS 앱 (재작성 진행 중 — ADR-0001)
...
5.4.1 런타임 및 언어
	•	백엔드 언어: TypeScript (Bun — Runner, Daemon, Relay, CLI) — 재작성 완료 전까지 레퍼런스 유지
	•	앱 언어: Swift (SwiftUI) — 재작성 진행 중 (ADR-0001)
	•	공유 코어: Rust (`tp-core`) — Phase 2에서 구현 예정 (codec/crypto/pairing)
	•	런타임: Bun v1.3.13+ (Runner, Daemon, Relay)
	•	배포(CLI): bun build --compile 으로 단일 바이너리 생성

5.4.2 PTY
	•	macOS/Linux: Bun.spawn({ terminal }) — Bun v1.3.13+ 네이티브 PTY
	•	Windows 네이티브 실행은 지원하지 않으며, Windows 사용자는 WSL 안에서 Linux 빌드를 사용한다.

5.4.3 앱 기술 (재작성 진행 중)
	•	UI 프레임워크: SwiftUI
	•	터미널 렌더링: Phase 3에서 결정 (SwiftTerm 또는 libghostty 네이티브 모듈 후보)
	•	빌드/검증: 로컬 Simulator 하니스 (`scripts/ios.sh` — xcodebuild + xcrun simctl)

5.4.4 암호화
	•	libsodium (X25519 키 교환 + XChaCha20-Poly1305 대칭 암호화)
	•	백엔드: libsodium-wrappers (WASM/Bun)
	•	Swift 앱: Phase 2에서 Rust `tp-core` UniFFI로 노출 예정
...
├── ios/              # Swift 앱 (SwiftUI — Phase 0 완료, 재작성 진행 중)
├── packages/
...
5.4.6 플랫폼 우선순위
	•	iOS (Simulator 우선) — 재작성 후 기준 플랫폼
	•	Web/Android: 재작성 이후 별도 검토
...
14.2.2 암호화 라이브러리
	...
	•	백엔드(Bun): libsodium-wrappers (WASM)
	•	Swift 앱: Phase 2에서 Rust `tp-core` UniFFI로 노출 예정 (byte-exact wire 호환 보장)
