...
5.4.3 Frontend 기술
	...
	•	터미널 렌더링: ghostty-web (libghostty WASM, Canvas 2D 렌더) — GhosttyTerminal.tsx(web) / GhosttyNative.tsx(native)
...
6.2 Terminal UI 목표
	...
	•	PTY raw bytes를 그대로 ghostty-web에 전달하여 ANSI escape 시퀀스(색상, 커서, 대체 화면 버퍼 등) 완벽 재현
...
Client→Relay (9종):
	...
	•	relay.push — daemon이 relay에 Expo push 전달 요청 (frontendId, token, title, body).
...
Relay→Client (10종):
	...
	•	relay.notification — frontend로 전달되는 Expo push 알림.
...
릴리즈 자동화:
	...
	•	release-please.yml은 workflow_dispatch 전용이며 dispatch당 정확히 한 동작(Release PR 생성/갱신 또는 v* 태그 push)만 수행 — 따라서 patch 릴리즈 한 번에 두 번 dispatch가 필요하다. release.yml(darwin-arm64 + linux x64/arm64 빌드 → GitHub Release → Homebrew tap bump)은 GitHub API 태그 push 이벤트 불안정(#172)으로 항상 manual dispatch로 트리거.

EAS / Web 배포:
	•	EAS preview.yaml(TestFlight / Android Internal)은 ci.yml의 eas-gate 잡이 5개 CI 잡 통과 + apps/app/**·packages/protocol/** 변경 감지 시 자동 실행(`eas workflow:run --ref`). preview.yaml 자체에는 on: 트리거가 없다.
	•	EAS production.yaml(App Store / Play Store)은 manual-only — on: 트리거 없음, release.yml이나 어떤 CI 이벤트로도 자동 실행되지 않는다.
	•	두 EAS 워크플로 모두 fingerprint 기반: 현재 fingerprint와 일치하는 빌드가 있으면 OTA update(type: update), 없으면 full build(type: build) 후 스토어 submit.
	•	Web(tpmt.dev)은 main push마다 Vercel이 완전 자동 배포(워크플로 파일 불필요).
...
14.2.2 암호화 라이브러리
	...
	•	Bun/웹 환경: libsodium-wrappers (WASM)
	•	Expo(Hermes) 환경: libsodium-wrappers (asm.js fallback) + expo-crypto polyfill (getRandomValues 주입). react-native-libsodium은 채택하지 않았다.
...
19. 비기능 요구사항
	...
	•	RN Web 포함 환경에서 터미널 렌더링과 입력 품질 확보 (ghostty-web)
...
21. MVP 단계 정의 (전부 출하 완료)

아래 Stage 0–5는 모두 출하(shipped)되었다. 이 절은 회고용으로, 각 단계가 무엇을 전달했는지 기록한다.

Stage 0: Foundation — 완료
...
Stage 3: Worktree + Session 관리 — 완료
...
검증 완료:
	...
	•	[해결] ANSI escape 시퀀스 — ghostty-web(libghostty WASM)이 터미널에서 네이티브 처리, Chat은 regex 기반 ANSI strip
...
추가 해결:
	...
	•	[해결] Hermes(iOS/Android)에서 libsodium WASM 미지원 — react-native-libsodium 대신 libsodium-wrappers(asm.js fallback) + expo-crypto polyfill(getRandomValues 주입, crypto-polyfill.ts) 채택
	•	[해결] 터미널 렌더러 — xterm.js WebView 브릿지 대신 ghostty-web(libghostty WASM, Canvas 2D) 채택. WebView 브릿지 불필요
