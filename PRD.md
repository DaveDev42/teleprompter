...
2.1 목표
	...
	•	Swift(SwiftUI) 기반 iOS 네이티브 앱을 지원한다 (Simulator 우선, 재작성 진행 중)
...
5.1 컴포넌트
	•	Frontend(앱): Swift(SwiftUI) Apple 멀티플랫폼 앱 (iOS/iPadOS/네이티브 macOS 출하, visionOS/watchOS는 ADR-0002 Phase B)
	•	Daemon: PC side(macOS/Linux; Windows 사용자는 WSL) 상주 에이전트. Rust 네이티브 바이너리(`tp-daemon`)로 배포.
...
5.4.1 런타임 및 언어 [완료 → 대체: Bun/TypeScript 백엔드는 "#5 zero-Bun cascade" PR6(#933)에서 전량 삭제, Bun/Node 툴체인은 PR7(#935)에서 제거. 아래는 현재 스택.]
	•	백엔드 언어: Rust (`rust/` 워크스페이스 — `tp-cli`, `tp-daemon`, `tp-runner`, `tp-relay`, `tp-proto`, `tp-core`). Bun/TypeScript 레퍼런스 구현은 삭제 완료, wire/E2EE byte-exactness는 TS 시절 골든벡터(`wire_vectors.rs`/`message_vectors.rs`)가 계속 잠근다.
	•	앱 언어: Swift (SwiftUI) — Apple 멀티플랫폼(iOS/iPadOS/네이티브 macOS 출하, visionOS/watchOS는 ADR-0002 Phase B)
	•	공유 코어: Rust (`tp-core`) — 구현 완료 (wire codec + E2EE crypto(AEAD/KDF/crypto_kx/ratchet) + pairing), UniFFI로 Swift에 노출
	•	배포(CLI): `cargo build --release` 로 4개 바이너리(`tp`/`tp-daemon`/`tp-runner`/`tp-relay`) 생성, `scripts/build-bundle.sh` 가 prefix-tree 번들(`tp-<suffix>.tar.gz`)로 조립

5.4.2 PTY
	•	macOS/Linux: `portable-pty`(wezterm) crate — Rust 네이티브 PTY (`rust/tp-runner/src/pty.rs`)
	•	Windows 네이티브 실행은 지원하지 않으며, Windows 사용자는 WSL 안에서 Linux 빌드를 사용한다.

5.4.3 앱 기술
	•	UI 프레임워크: SwiftUI
	•	터미널 렌더링: SwiftTerm
	•	빌드/검증: 로컬 하니스 (`scripts/ios.sh` — xcodebuild + xcrun simctl(iOS/visionOS/watchOS) / `open` + 호스트 unified log(macOS), `TP_PLATFORM=ios|macos|visionos|watchos`)

5.4.4 암호화
	•	libsodium 호환 (X25519 키 교환 + XChaCha20-Poly1305 대칭 암호화)
	•	백엔드: Rust `tp-core` (순수 Rust crate — `chacha20poly1305`, `x25519-dalek`, `blake2`; TS 시절 libsodium-wrappers(WASM) 구현과 골든벡터로 byte-exact 검증 완료)
	•	Swift 앱: Rust `tp-core` UniFFI로 노출 완료 (byte-exact wire 호환)

5.4.5 모노레포
	•	도구: cargo workspace (Rust) — Turborepo/pnpm은 PR7에서 Bun/Node 툴체인과 함께 제거
	•	구조:

teleprompter/
├── ios/              # Swift(SwiftUI) 앱 — project.yml(XcodeGen SoT), Sources/, Tests/, Generated/(UniFFI, gitignored)
├── rust/             # Rust 워크스페이스 (백엔드/CLI 전체)
...
│   ├── tp-cli/       # `tp` 바이너리 — subcommand 라우터 + passthrough terminal-proxy
│   ├── tp-daemon/    # 장기 실행 서비스 (session mgmt, vault, E2EE, worktree)
│   ├── tp-runner/    # 세션당 프로세스 (claude PTY, hooks 수집)
│   ├── tp-relay/     # WebSocket relay (암호화된 프레임만 중계)
│   ├── tp-loopback/  # smoke 하니스용 in-process relay + 가짜 daemon peer
│   └── tp-e2e-holder/ # 로컬 실-claude E2E holder
├── scripts/
│   ├── ios.sh        # 로컬 Apple 하니스 (rust→gen→build→install→launch→smoke/test)
│   ├── build-bundle.sh  # release bundle 조립
│   └── install.sh    # curl-pipe-sh installer
└── version.txt       # 단일 버전 SoT (release-please)

5.4.6 플랫폼 우선순위
	•	Apple 멀티플랫폼 — iOS/iPadOS/네이티브 macOS 완전 경험 (Phase A, 출하), visionOS 완전 + watchOS 제한 경험 (Phase B, ADR-0002)
	•	Web/Android: 재작성 이후 강등 (ADR-0001 §6 확장 경로 유지)
...
6.2 Terminal UI 목표
	...
	•	PTY raw bytes를 터미널 렌더러에 전달하여 ANSI escape 시퀀스(색상, 커서, 대체 화면 버퍼 등) 완벽 재현
...
앱 (Swift — 재작성 진행 중, Phase 3에서 기능 parity 목표)
	•	Session 목록/선택
	•	Chat(Activity) 렌더링
	•	Terminal 렌더링
	•	QR 페어링
...
8.2 Transport 원칙
	...
	•	대상 환경은 iOS(Swift URLSessionWebSocketTask) + 백엔드 Bun
...
10.3 Frame type (Envelope)

Envelope.t의 기본 frame type (packages/protocol/src/types/envelope.ts):
	•	hello
	•	attach
	•	detach
	•	resume
...
Frontend↔Daemon 제어/데이터 프레임은 session-proto.ts의 타입드 union(SessionClientMessage / SessionServerMessage)으로 별도 정의된다 — resize, worktree.create/remove/list, session.create/stop/restart/export, pushToken 등은 이 union에만 존재하며 위 FrameType 리터럴에는 없다.

10.4 Relay Protocol v2 wire 메시지

Relay는 ciphertext-only zero-trust 중계자이므로, relay wire 메시지는 위 Envelope와 별도의 메시지 집합이다. 두 방향 모두 런타임 가드(parseRelayClientMessage / parseRelayServerMessage)가 field-by-field 검증하며 unknown type은 drop된다. 전체 wire 상세(resume token 동작, 버전 게이팅)는 .claude/rules/protocol.md 가 SoT.

Client→Relay (9종):
	•	relay.register — daemon self-register (proof = BLAKE2b(pairingSecret‖"relay-register"), token = BLAKE2b(pairingSecret‖"relay-auth")). relay.auth 이전에 보낸다.
	•	relay.auth — token 인증. role(daemon|frontend), daemonId, frontendId?(frontend N:N 멀티플렉싱).
	•	relay.auth.resume — HMAC 서명 토큰 fast-path 재연결. relay 재시작 생존(TP_RELAY_RESUME_SECRET 설정 시). 실패 시 full register+auth로 fallback.
	•	relay.kx — in-band pubkey 교환. ct = deriveKxKey(pairingSecret)로 암호화된 공개키. relay는 복호화하지 않고 relay.kx.frame으로 전달.
	•	relay.pub — E2EE 데이터 프레임 발행 (sid, ct = XChaCha20-Poly1305 ciphertext, seq).
	•	relay.sub / relay.unsub — 세션 구독/해제 (after? = 누락 프레임 replay).
	•	relay.ping — keepalive (daemon 30초 주기, 3회 누락 = 90s idle close).
	•	relay.push — daemon이 relay에 push 전달 요청 (frontendId, token, title, body).

Relay→Client (10종):
	•	relay.auth.ok — 인증 수락. resumeToken?, resumeExpiresAt?, resumed?(true면 fast-path resume → daemon은 relay.kx 재브로드캐스트 생략).
	•	relay.auth.err / relay.register.err — 거부. client는 full auth로 fallback.
	•	relay.register.ok — 토큰 등록 수락.
	•	relay.frame — 전달된 암호 데이터 프레임 (from, frontendId? when from=frontend).
	•	relay.kx.frame — 전달된 키교환 봉투 (relay 복호화 안 함).
	•	relay.presence — daemon online/offline + sessions + lastSeen.
	•	relay.pong — keepalive 응답.
	•	relay.err — 일반 relay 오류.
	•	relay.notification — 앱으로 전달되는 push 알림.

제어 평면 (E2EE peer-to-peer, __control__ 가상 sid):
	•	control.unpair — 페어링 제거 (daemonId, frontendId, reason, ts). relay.pub ciphertext 내부로 흐르며 relay는 보지 못함. stateless — peer offline 시 유실.
	•	control.rename — label 갱신 (label = Label tagged union { set: true; value } | { set: false }). { set: false }는 권위적 clear(legacy "" sentinel 대체). daemon은 peer 버전에 따라 wire shape를 게이팅(미업데이트 앱에 legacy string)해 무음 clear를 방지.

E2EE 키 유도: kxKey = BLAKE2b(pairingSecret‖"kx-envelope"). 세션키 = X25519 crypto_kx(daemon=server, frontend=client) → BLAKE2b-HKDF ratchet으로 tx/rx 분리. 프레임 암호화 = XChaCha20-Poly1305, 프레임별 랜덤 24B nonce, wire = base64(nonce‖ciphertext).
...
실행 / passthrough:
	...
	•	tp -- <claude args> — daemon 없이 claude로 직접 포워딩.
	•	tp --help|-h / --version|-v — tp 도움말·버전을 claude의 것과 합쳐 출력.
	•	passthrough 전용 플래그: --tp-sid <id>, --tp-cwd <path> (나머지는 모두 claude로 전달).

페어링:
	•	tp pair [new] [--relay URL] [--label NAME] — QR 생성 후 frontend ECDH 완료까지 블록.
	•	tp pair list — LABEL/DAEMON ID/RELAY/CREATED 컬럼.
	•	tp pair rename <id-prefix> <label...> — label 변경 (daemon 가동 시 IPC 경유 control.rename으로 peer 통지).
	•	tp pair delete <id> [-y] — 페어링 삭제 (daemon 가동 시 control.unpair 통지).

세션:
	•	tp session list — SID/STATE/CWD/UPDATED 컬럼.
	•	tp session delete <sid> [-y] — running이면 Runner kill 후 삭제.
	•	tp session cleanup [-y] [--all] — TTY 대화형 multi-select 일괄 삭제 (stopped 한정).
	•	tp session prune [--older-than Nd|Nh|Nm|Ns] [--all] [--running] [--dry-run] [-y] — non-interactive 일괄 삭제.

상태 / 진단:
	•	tp status — daemon·세션 상태 (cwd/worktree 그룹).
	•	tp logs [sid] — 세션 라이브 출력 tail (500ms 폴링).
	•	tp doctor — 환경 진단 + relay 연결 + E2EE self-test, 끝에서 `claude doctor`.
	•	tp upgrade — GitHub 릴리즈에서 바이너리 갱신(SHA-256 검증) 후 daemon 재기동, `claude update`. Homebrew 설치는 `brew upgrade` 힌트만 출력.
	•	tp version — tp 버전 + claude 버전.

Daemon / Relay / 완성:
	•	tp daemon start|stop|status|install|uninstall — OS 서비스(launchd/systemd --user) 등록·해제 포함.
	•	tp relay start [--port 7090] [--cache-size N] [--max-frame-size N] — relay 서버 실행.
	•	tp completions <bash|zsh|fish> / install [shell] / uninstall [shell] — 셸 자동완성.

Claude 유틸리티 (daemon 없이 직접 포워딩): tp auth, mcp, install, update, agents, auto-mode, plugin, plugins, setup-token.

내부 전용: tp run (daemon이 per-session Runner를 띄울 때 사용, help/문서 비노출).

환경 변수: TP_NO_UPDATE_CHECK=1 (startup 버전 체크 억제), TP_NO_AUTO_INSTALL=1 (first-run 서비스 설치 프롬프트 스킵). 백그라운드 업데이트 체크는 passthrough·upgrade·doctor·pair에서만 발화.

10.6 배포 모델

**EAS 클라우드 빌드 / Expo OTA 는 제거됨** (ADR-0001 Phase 1 — Expo 앱 삭제 완료).

tp CLI 바이너리 릴리즈:
	•	릴리즈 태그는 v*(예: v0.1.19) — release/v* 접두사 없음(include-component-in-tag: false). release-type: node, bump-patch-for-minor-pre-major: true(pre-1.0에서 feat:도 patch).
	•	release-please.yml은 workflow_dispatch 전용이며 dispatch당 정확히 한 동작(Release PR 생성/갱신 또는 v* 태그 push)만 수행 — 따라서 patch 릴리즈 한 번에 두 번 dispatch가 필요하다.
	•	release.yml(darwin-arm64 + linux x64/arm64 빌드 → GitHub Release → Homebrew tap bump)은 GitHub API 태그 push 이벤트 불안정(#172)으로 항상 manual dispatch로 트리거.

Swift 앱 배포:
	•	현재: 로컬 Simulator 하니스(`scripts/ios.sh`)만 존재. App Store/TestFlight 배포는 Phase 3 이후 별도 결정.
	•	Web(tpmt.dev) 및 Android 배포: 재작성 이후 미결.
...
14.2.2 암호화 라이브러리
	•	libsodium 호환 사용 (X25519 + XChaCha20-Poly1305)
	•	백엔드: Rust `tp-core` (순수 Rust crate — `chacha20poly1305`, `x25519-dalek`, `blake2`; TS 시절 libsodium-wrappers(WASM) 구현과 골든벡터 byte-exact 검증 완료, Bun 백엔드는 #5 PR6에서 삭제)
	•	Swift 앱: Rust `tp-core` UniFFI로 노출 완료 (byte-exact wire 호환 보장)
...
15.6 API Key 관리
	...
	•	iOS: Keychain (Swift Keychain API)
...
진단 정보의 밀도는 모바일 / 태블릿 / PC 레이아웃에 따라 조정한다 (Swift 앱 Phase 3에서 구현 예정).
...
19. 비기능 요구사항
	...
	•	iOS 환경에서 터미널 렌더링과 입력 품질 확보 (터미널 렌더러 Phase 3 결정)
...
21.1 Expo 스택 단계 (완료 → 재작성으로 대체)

아래 Stage 0–5는 Expo 스택에서 완료(shipped)되었으나, ADR-0001 결정에 따라 Swift 재작성으로 대체된다. 회고 기록.

Stage 0: Foundation — 완료 (백엔드, 재작성 후에도 유지)
...
Stage 1: Local UI — 완료 (Expo — 재작성 대상)
	•	Expo 프로젝트 초기화 (React Native + RN Web) ← 제거됨 (Phase 1)
	•	Terminal 탭: ghostty-web 기반 터미널 렌더링
	•	Chat 탭: hooks 이벤트 기반 메시지 카드
	•	앱 ↔ Daemon 통신은 항상 relay 경유 — 직접 WebSocket 연결 경로는 존재하지 않는다 (Architecture Invariant)

Stage 2: Relay + E2EE — 완료 (백엔드, 재작성 후에도 유지)
...
Stage 3: Worktree + Session 관리 — 완료 (백엔드, 재작성 후에도 유지)
	•	Daemon의 git worktree 직접 관리 (add/remove/list)
	•	다중 Session 지원 (worktree당 N개 Session)

Stage 4: Voice UX — 완료 (Expo — 재작성 대상)
	•	OpenAI Realtime API 연동 (WebSocket)
	•	연속 청취 모드, VAD 자동 감지, TTS 응답
	•	컨텍스트 주입 (Chat 요약 + Terminal 상태)

Stage 5: Mobile + Responsive — 완료 (Expo — 재작성 대상)
	•	iOS 빌드 (EAS) ← 제거됨. 로컬 Simulator 하니스로 대체
	•	오프라인 상태 복원, 재연결 안정화

21.2 재작성 단계 (ADR-0001 — 진행 중)

Phase 0 (완료): Swift→Simulator 하니스
	•	최소 SwiftUI 앱이 xcodebuild로 빌드되고 simctl install/launch로 Simulator에서 부팅
	•	scripts/ios.sh 재실행 스크립트 구축

Phase 1 (진행 중): Expo + EAS 정리
	•	apps/app(Expo) 제거 ✓, eas.json/.eas/ 제거 ✓, e2e/(Playwright) 제거 ✓
	•	관련 docs·rules 갱신 진행 중

Phase 2 (미착수): tp-core (Rust) + Swift FFI
	•	codec/crypto/pairing 구현 + UniFFI xcframework
	•	Swift 앱에 링크, encode→encrypt→decrypt→decode 라운드트립 Simulator 검증

Phase 3 (미착수): Swift 앱 기능 parity
	•	pairing(QR), relay client(WS), 세션 목록, Chat(hooks), 터미널(SwiftTerm/libghostty)

Phase 4 (미착수): 백엔드 Rust 이관
	•	relay→daemon→runner, wire 호환 유지하며 컷오버
...
검증 완료:
	...
	•	[해결] ANSI escape 시퀀스 — Expo 스택에서 ghostty-web(libghostty WASM)이 처리. Swift 재작성에서 터미널 렌더러 재결정 필요 (Phase 3)

추가 해결 (백엔드 — 재작성 후에도 유지):
	...
	•	[해결/Expo 이력] Hermes에서 libsodium WASM 미지원 — libsodium-wrappers(asm.js fallback) + expo-crypto polyfill 채택. Swift 재작성에서는 Rust tp-core UniFFI로 대체 예정.
	•	[해결/Expo 이력] 터미널 렌더러 — ghostty-web(libghostty WASM, Canvas 2D) 채택. Swift 재작성에서 터미널 렌더러 재결정 (Phase 3).

미해결:
	•	음성 입력 모드에서 토글/옵션의 최종 레이아웃 (Phase 3)
	•	hook 기반 카드 휴리스틱의 실제 규칙 테이블 (Phase 3)
	•	PTY output 파싱 규칙의 Claude Code 버전별 분기 전략 상세 (Phase 3)
	•	공식 호스팅 relay의 운영 정책 (SLA, 리전, 요금)
	•	Swift 앱 터미널 렌더러 엔진 선택 (SwiftTerm vs libghostty 네이티브 — Phase 3 결정)
	•	Swift 앱 App Store/TestFlight 배포 전략 (Phase 3 이후 결정)
	•	Web/Android 지원 우선순위 (재작성 이후 별도 검토)
