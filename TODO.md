...
## 🚀 피봇 (ADR-0001) — Apple 집중 + Rust 백엔드 점진 전환

> 결정 SoT: `docs/adr/0001-rust-backend-incremental-native-app.md`. **전면 재작성 아님** —
> 두 독립 트랙. 세부 항목은 GitHub Project 에서 추적. 각 Rung/Phase 독립 출시·롤백 가능.

### 트랙 A — 앱 (RN 셸 유지, 터미널만 네이티브화 · `docs/native-terminal-plan.md`)
- [x] **Rung 0** — 결정 + `TermHandle` seam 정리 (2026-06-12, PR #626)
- [x] **Rung 1** — WebView 경로 de-risk: ghostty-web UMD 로컬 에셋 번들 (2026-06-12, JS-only/OTA, 검증 큐 Q13)
- [ ] **Rung 2** — 플래그 뒤 네이티브 모듈 스캐폴드 (`TPTerminalView` Expo Module, 첫 fingerprint break, `ignorePaths` 정비)
- [ ] **Rung 3** — 엔진 연결(**SwiftTerm** 확정) write/input/resize parity + initial-size(120×40) + ready-전 버퍼링
- [ ] **Rung 4** — search + voice(`getTerminalLines`) + 라이프사이클 parity (bg/fg, scrollback 10000)
- [ ] **Rung 5** — 네이티브 기본 전환 + WebView 은퇴(iOS, 한 릴리즈 escape hatch 후 삭제)
- [ ] **Rung 6** (선택) — libghostty 태그 릴리즈 후 엔진 스왑 (Metal 회복) / Android 엔진 재평가
- [ ] **watchOS 컴패니언** (Phase 1 후, 별도 Swift WatchKit — Expo 미지원): 세션 상태 glance +
      Stop 알림 + 권한요청 Approve/Deny. 페어링은 `WatchConnectivity` 로 iPhone 시크릿 전달(QR 불필요).
      크립토는 libsodium xcframework(XChaCha 24B nonce — CryptoKit 불가). 터미널·Chat·voice 제외.

### 트랙 B — 백엔드 (Rust 코어 + UniFFI · ADR §3)
- [ ] **Phase 0 — crypto/UniFFI 스파이크 (Go/No-Go, 코드만/출하 없음)**: 최소 `tp-core` crate
      (XChaCha20-ietf 24B nonce AEAD + BLAKE2b-512 kx-derive + framed codec) → ① Swift UniFFI round-trip
      ② 기존 Bun daemon 과 실제 페어링 왕복. 둘 다 통과 = 트랙 B Go.
- [ ] **Phase 1** — `tp-core` 정식화 + Swift 앱 네이티브 모듈이 링크 (트랙 A Rung 3 과 합류 가능)
- [ ] **Phase 2** — relay Rust(tokio) + transport WS→QUIC/HTTP3(quinn/tokio-quiche) 평가
      (HoL blocking 제거, connection migration → dead-pairing throttle/heartbeat 우회 대체, 0-RTT). 10k capacity QUIC 재측정 필수.
- [ ] **Phase 3** — daemon/runner Rust (성능/메모리 본진). 선행: CLI 의 `@teleprompter/daemon` Store 직접 import 를 IPC 로 단절.
- [ ] **CLI** — TypeScript 유지 결정 (ADR §4-②). Rust 화는 daemon 안정화 후 재평가.
