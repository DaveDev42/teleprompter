...
- [ ] **`tp-relay` push ws-verdict TOCTOU 재검증 미이식 (TS parity gap, #5 PR6 문서 검증 중 발견)** — TS 레퍼런스는 `relay-server.ts:1594-1618` 에서 unseal/sendOrDeliver await 뒤 `isFrontendWsLive` 로 stale `"ws"` verdict 를 재검증해 APNs 로 re-deliver 했다. Rust `tp-relay` 는 `send_or_deliver` step 1 이 dispatch 시점에 pre-sample 된 `is_frontend_connected` 로 즉시 `Ws` 를 반환하고 post-await 재검증이 없다 (`rust/tp-relay/src/push.rs:360-366`, `conn.rs handle_push` spawn 경로) — 샘플링과 in-band 전달 사이에 frontend 가 끊기면 그 알림은 APNs fallback 없이 드랍된다. 창은 TS(unseal await 포함)보다 훨씬 작지만 0 이 아님. 수정 = Ws verdict 전달 직전 liveness 재확인 + 죽었으면 APNs arm 재진입 + 회귀 가드 (`conn.rs`). `.claude/rules/native-testing.md` PUSH flake caveat 절 참조.
- [ ] **`session.export` 음수 `limit` 이 50k cap 을 우회 (TS guard 미이식, #5 PR8 문서 검증 중 발견)** — `handle_relay_session_export`(`rust/tp-daemon/src/ipc/command_dispatcher.rs:1671`)는 `limit` 을 bare `Value::as_i64` 로 읽고, clamp 는 `:1689` 의 `unwrap_or(50_000).min(50_000)` + `session_db.rs:134` 의 동일 re-clamp 뿐 — `.min()` 은 상한만 자르므로 음수(예: `-2`)가 산술 그대로 살아남아 SQLite `LIMIT -1` = **무제한**으로 도달한다. TS 의 `isOptionalPositiveInt` guard 가 막던 정확히 그 bypass. 수정 = dispatch 지점에서 non-positive `limit` reject (`tp_proto::is_positive_int` 재사용) + 음수 입력 회귀 테스트. `.claude/rules/protocol.md` "Wire-boundary Guards" 참조.
- [ ] **`relay.err` 구조화 `frontendId` + daemon-side dead push-token eviction 미이식 (#5 PR8 문서 검증 중 발견)** — `RelayErr`(`rust/tp-relay/src/messages.rs:133-137`)는 `{e, m}` 뿐이라 `PUSH_TOKEN_DEAD`/`PUSH_UNSEAL_FAILED` 의 대상 fid 가 사람이 읽는 `m` 문자열 안에만 있고, daemon `handle_relay_err`(`relay_client.rs:725-759`)는 per-frontend eviction 을 수행하지 못한다("no eviction" 로그) — dead token 은 frontend 의 다음 재연결 re-register 까지 매 이벤트마다 실패 push 를 재전송한다. 수정 = `relay.err` 에 additive-optional `frontendId` 필드 추가(구 daemon 은 무시 — WS v-additive 원칙) + daemon 측 store token 제거 + 양측 회귀 가드. `.claude/rules/relay-capacity.md` "Dead APNs token eviction" 참조.
- [ ] **`tp pair list`/`rename` 이 peer-supplied label 을 raw 출력 (TS `stripAnsi` display-sanitization 미이식)** — TS CLI 는 표시 직전 ANSI/control char 를 strip 했다(`lib/sanitize.ts`). Rust `tp-cli` 는 `commands/pair.rs` 렌더에서 label 을 그대로 print — 적대적 peer rename 이 터미널 escape 를 주입할 수 있다. 수정 = 표시 전용 sanitize 헬퍼 포트(저장값은 불변) + 회귀 테스트. 코드 주석으로 위치 표시됨.
- [ ] **`tp-core` `pairing.rs` 하드닝 경로 직접 테스트 부재** — `MAX_PAIRING_B64_LEN`(2048 pre-decode cap, `pairing.rs:28,194`) 과 did/relay/hostname strict-UTF-8 거부(`:221,237,282`)는 라이브 코드지만 그 경계를 정면으로 잠그는 유닛/골든벡터 케이스가 없다 (over-cap 입력, invalid-UTF-8 필드). 각 1–2 케이스면 충분.
...
- [x] **Phase 4: 백엔드 Rust 이관** — [ADR-0003](docs/adr/0003-phase4-backend-rust-migration.md) **Accepted (2026-06-17, Dave)**, staged dual-run cutover. **✅ 완료 (2026-07-23): 전 조각(relay/CLI/runner/daemon) 포팅 + #4 flip(PR #922, 소크 5/5) + #5 Bun 삭제 캐스케이드 전체 머지 — PR1 #925/PR2a #926/PR3 #927/PR4 #928/PR5 #929/PR2b #932/PR6 #933/PR7 #935(toolchain 제거 + version.txt SoT)/PR8 #936(docs prose 정리). Rust 워크스페이스가 유일 구현.**
  ...
  - [x] **runner** (`packages/runner`, 763 LOC, PTY + hooks) ✅ **포팅 완료** (`tp-runner`, inc1~4 머지 #882/#883/#884/#890, dual-run seam `TP_RUNNER_BIN` + 실 claude 파리티 게이트 `TP_E2E_RUNNER_BIN`). **#4 flip 머지 완료**(PR #922): daemon 이 `tp_proto::locate_tp_runner()` 로 Rust `tp-runner` 를 기본 spawn(`["bun","run"]` placeholder 제거).
  ...
  - [x] **daemon** (`packages/daemon`, 5.4k LOC, session/store/relay-client/pairing/push/worktree) ✅ **포팅 완료** (`tp-daemon`, inc1~6 머지 #892~#897): store(rusqlite, 포맷 동결) → ipc/session/worktree → relay-client → pairing/push/relay-manager → dispatcher + shipping `[[bin]]` + `TP_DAEMON_BIN` dual-run seam. flip-prep A1(#898, `locate_tp_daemon` + release 아티팩트) + A2(`TP_E2E_DAEMON_BIN` 파리티 게이트). **#4 flip 머지 완료**(PR #922): `ensure_daemon.rs` background + `commands/daemon.rs::start` foreground/service 둘 다 `locate_tp_daemon()` → Rust daemon. CI-결정론 differential 파리티 = 5× `*-rust-parity.test.ts`(store/worktree/transport/push/ipc).
  - 참고: `tp-core`(crypto+codec+pairing)는 Phase 2 에서 byte-exact 완료(앱에 링크). **PTY 크레이트 spike 는 증분 1 에서 portable-pty 로 확정·해소**. store(rusqlite 포맷 동결)·worktree 리스크는 daemon 포팅에서 해소됨. **#5 Bun 삭제 캐스케이드 완료** (PR2b holder 포트 #932 ✅ → PR6 `packages/*`+`apps/cli`+`tpd` 삭제 #933 ✅ → PR7 toolchain 제거+version.txt SoT #935 ✅ → PR8 문서 정리 #936 ✅).
