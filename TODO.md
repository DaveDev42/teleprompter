...
- [ ] **Phase 4: 백엔드 Rust 이관** — [ADR-0003](docs/adr/0003-phase4-backend-rust-migration.md) **Accepted (2026-06-17, Dave)**, staged dual-run cutover. **진행 상태 (실제):**
  ...
  - [ ] **runner** (`packages/runner`, 763 LOC, PTY + hooks) — 포팅 **진행 중** (`tp-runner`, dual-run seam `TP_RUNNER_BIN`, cutover 없음).
    ...
    - [ ] **증분 2 — async 오케스트레이션** — tokio IPC 클라이언트(framed codec, decode-throw teardown 불변식) + hook receiver(UnixListener, sid traversal 가드, UTF-8 버퍼 cap) + `Runner` 상태머신(hello/bye pid 생성 가드, graceful shutdown bye-flush) + `main.rs` argv 파싱. 그다음 daemon `TP_RUNNER_BIN` seam 으로 단일 세션 dual-run.
