...
- [x] **`tp pair list`/`rename` 이 peer-supplied label 을 raw 출력 (TS `stripAnsi` display-sanitization 미이식)** ✅ — `commands/pair.rs` 에 표시 전용 `sanitize_label_for_display`(CSI/OSC/2-char escape/control 문자 strip; 저장값 불변) 포트, `pair list` 테이블 + `pair rename` 에코 양쪽 적용. 회귀 테스트 5종 (CSI/OSC 양 종결자/미종결 시퀀스/bare control/유니코드 통과).
- [x] **`tp-core` `pairing.rs` 하드닝 경로 직접 테스트 부재** ✅ — `rejects_over_cap_payload_before_decode`(cap+1/10×cap 거부 + 정상 번들 headroom) + `rejects_invalid_utf8_in_string_fields`(did/relay/hostname 각각 invalid-UTF-8 주입 거부 + valid 대조군) 추가.
