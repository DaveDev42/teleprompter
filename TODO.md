...
- [ ] **Long-running 안정성 (1시간 soak)** — daemon 메모리 RSS 추이, 100회 relay reconnect, 100개 frame round-trip latency, WS idle/wake cycle 5회. **측정 스크립트 완료** (`scripts/soak.ts` — in-process RelayServer + 실 daemon pid RSS 샘플링, `bun run scripts/soak.ts` 로 1h soak, `--minutes`/`--round-interval`/`--reconnects`/`--frames`/`--idle-cycles`/`--idle-hold`/`--json` 플래그, hard failure 시 exit 1). 남은 건 실제 1h 실행 + 결과 기록. (1시간 실행)
