...
- [ ] **Phase 4: 백엔드 Rust 이관** — [ADR-0003](docs/adr/0003-phase4-backend-rust-migration.md) **Accepted (2026-06-17, Dave)**, staged dual-run cutover. **진행 상태 (실제):**
  ...
  - [ ] **runner** (`packages/runner`, 763 LOC, PTY + hooks) — 미포팅, Bun/TS. **다음 증분** (ADR 순서상 runner → daemon).
  - [ ] **daemon** (`packages/daemon`, 5.4k LOC, session/store/relay-client/pairing/push/worktree) — 미포팅, Bun/TS. 최대·최종 조각.
  - 참고: `tp-core`(crypto+codec+pairing)는 Phase 2 에서 byte-exact 완료(앱에 링크). 남은 리스크는 daemon 의 store(rusqlite 포맷 동결)·worktree(git2 vs shell)·PTY 크레이트 spike (ADR §6).
