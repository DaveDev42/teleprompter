...
### Windows (dropped — see "Drop native Windows support" below)
- [x] ~~Bun `bun:sqlite` Windows finalizer 지연~~ — moot. Native Windows support was dropped; Windows users run the Linux build inside WSL where the SQLite finalizer behavior matches Linux. Entry point now exits with a WSL pointer on `process.platform === "win32"`.
- [x] ~~`checkForUpdates` Windows CI flake~~ — fixed via `fetchLatest` DI in PR #154 era; subsequently the entire Windows CI matrix was removed when native Windows support was dropped.
...
- [ ] **macOS 바이너리 Developer ID signing + notarization** — 현재 `tp` (bun SEA) 는 Jarred Sumner (bun runtime) 의 서명을 그대로 상속. 사용자가 launchd daemon 을 등록하면 macOS Login Items & Extensions UI 에 "Jarred Sumner" entry 가 표시되는 UX 이슈. 0.x 에서는 S1 (수용) 로 합의했고, 1.0 블로커로 승격 예정. 필요: (a) Apple Developer Program 가입 ($99/yr), (b) release.yml 에 `codesign` + `notarytool` + `stapler` 단계 추가, (c) darwin 바이너리에 자체 Developer ID 서명 후 notarize. 영향 범위는 macOS darwin_arm64/x64 릴리즈 아티팩트만. Linux/Windows 는 무관. **기술 검증 완료 (Bun 1.3.13)**: `bun build --compile` 결과물에 `--remove-signature` → Hardened Runtime + JIT entitlements (`com.apple.security.cs.allow-jit`, `allow-unsigned-executable-memory`, `disable-library-validation`) 로 재서명 → `codesign --verify` 통과 확인. 1.3.12의 truncated-signature 회귀 ([oven-sh/bun#29120](https://github.com/oven-sh/bun/issues/29120))는 1.3.13에서 fix됨. Apple Developer Program 구독만 남음.
