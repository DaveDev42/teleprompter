...
## 🐛 QA 발견 (2026-05-10 end-to-end QA, branch `fix/cli-sqlite-busy`)

`tp → daemon → relay → React Native Web` 전체 플로우 QA 중 발견. 우선순위 순.

### P0 — 차단성 버그 (수정 완료, 미커밋)

- [x] **SQLITE_BUSY: daemon이 살아있으면 모든 CLI 커맨드가 실패** — `packages/daemon/src/store/schema.ts`의 `journal_mode = DELETE`가 단일-writer rollback journal을 사용해, daemon이 writer lock을 들고 있으면 `tp pair list` / `tp status` / `tp session list` 등 단순 read도 `SQLITE_BUSY`로 즉시 throw. 짧은 CLI 프로세스가 daemon과 같은 sqlite 파일을 안전히 공유할 수 없음. **수정**: `journal_mode = WAL` + `busy_timeout = 5000` (이 PR — `fix/cli-sqlite-busy`). 검증: daemon running 상태에서 pair list/status/session list 모두 RC=0. **남은 작업**: 회귀 테스트 추가 (daemon writer + 동시 CLI reader 시나리오).

### P1 — 사용자 가시 동작 버그

- [ ] **`tp pair new` "disconnect" 토스트 오발화** — 페어링 정상 완료 직후에도 onClose 핸들러가 resolve 와 race 해서 가짜 disconnect 에러를 토스트로 띄움. `apps/cli/src/commands/pair.ts:170-174` 근처. resolve 시 onClose unhook 또는 race-free flag로 가드.
...
- [ ] **passthrough 세션 Chat 탭 무반응** — passthrough로 시작한 세션을 React Native Web Chat 탭에서 열면 Claude 응답이 streaming되지 않음. Terminal 탭에서 클로드의 INSERT-mode 에디터로 진입하는 시나리오에서 재현. hooks event 발화 vs PTY 파싱 hybrid 경로 중 어느 쪽에서 끊기는지 trace 필요.
- [ ] **Sessions 탭이 페어링 직후 비어 보임 (production relay 한정)** — 로컬 relay에서는 정상이지만 `wss://relay.tpmt.dev` 페어링 시 sessions 탭이 빈 채로 남는 케이스 관찰. presence 메시지 race 또는 frame replay 윈도우 (cacheSize=10) 부족 의심.
