...
- [ ] **Phase 4: 백엔드 Rust 이관** — [ADR-0003](docs/adr/0003-phase4-backend-rust-migration.md) **Accepted (2026-06-17, Dave)**, staged dual-run cutover. **진행 상태 (실제): 전 조각(relay/CLI/runner/daemon) 포팅 완료 + #4 flip 머지 완료(PR #922, 소크 5/5 통과). 잔여 = #5 Bun 삭제 캐스케이드 — PR1 #925/PR2a #926/PR3 #927/PR4 #928/PR5 #929/PR2b #932/PR6 #933/PR7 #935(toolchain 제거 + version.txt SoT) 머지, PR8(docs prose 정리) 진행 중.**
  ...
  - 참고: `tp-core`(crypto+codec+pairing)는 Phase 2 에서 byte-exact 완료(앱에 링크). **PTY 크레이트 spike 는 증분 1 에서 portable-pty 로 확정·해소**. store(rusqlite 포맷 동결)·worktree 리스크는 daemon 포팅에서 해소됨. **남은 것 = #5 Bun 삭제 캐스케이드 잔여 단계** (PR2b holder 포트 #932 ✅ → PR6 `packages/*`+`apps/cli`+`tpd` 삭제 #933 ✅ → PR7 toolchain 제거+version.txt SoT #935 ✅ → PR8 문서 정리 진행 중).
