...
- [ ] **Phase 4: 백엔드 Rust 이관** — [ADR-0003](docs/adr/0003-phase4-backend-rust-migration.md) **Accepted (2026-06-17, Dave)**, staged dual-run cutover. **진행 상태 (실제):**
  ...
  - [ ] **runner** (`packages/runner`, 763 LOC, PTY + hooks) — 포팅 **진행 중** (`tp-runner`, dual-run seam `TP_RUNNER_BIN`, cutover 없음).
    ...
    - [ ] **증분 3 — daemon `TP_RUNNER_BIN` seam** — daemon SessionManager 가 세션별로 Rust runner 바이너리를 고르게(env/플래그) → 단일 세션 dual-run + io-record 바이너리 사이드카 parity 게이트로 byte-exactness 검증.
