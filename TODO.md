...
- [ ] 네이티브 GPU 터미널 RN 모듈 — WebView 제거. 설계 맵 + 증분 사다리(Rung 0–6) =
      `docs/native-terminal-plan.md` (2026-06-12). **엔진 미결**: libghostty 는 무태그
      릴리즈/불안정 C API/iOS experimental (커뮤니티 wrapper 경유만 가능), SwiftTerm 은
      안정적이지만 Metal 아님 + web 과 엔진 분기 — Dave 결정 필요 (plan §6). Rung 2+ 는
      fingerprint break — Apple Watch 등 다른 네이티브 변경과 배칭.
      **Rung 0 ✅** (2026-06-12, PR #626 — TermHandle seam). **Rung 1 ✅** (2026-06-12 —
      ghostty-web UMD 로컬 에셋 번들, esm.sh 의존 + 0.3/0.4 skew 제거, write 브릿지
      base64 화로 Uint8Array 버그 수정. JS-only/OTA. 온디바이스 검증 = 큐 Q13).
