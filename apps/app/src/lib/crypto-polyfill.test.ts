 ...
 *
 * crypto-polyfill emits the `[tp-app boot]` marker and installs
 * `self.crypto.getRandomValues` (Hermes has neither `window` nor `self`).
 * We stub `expo-crypto.getRandomValues` so the side-effectful module can be
 * imported under Bun, then assert the polyfill behaves correctly.
 *
 * The libsodium wasm2js workarounds this module used to install (console
 * init-noise filter, WebAssembly stub, rejection-tracker re-registration)
 * were removed when the native CryptoProvider was enabled — libsodium never
 * evaluates on Hermes anymore. See crypto-provider-native.test.ts for the
 * cross-provider interop oracle.
 */
...
// test file runs. Bun provides `self` and `crypto` on globalThis already; to
// exercise the "bare Hermes" branches we hide them before importing the
// polyfill, then restore them afterwards.
