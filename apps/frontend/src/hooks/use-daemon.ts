...
export function useDaemon(url?: string) {
  ...
  const { setConnected, setSid, setLastSeq } = useSessionStore.getState();
  ...
  useEffect(() => {
    const client = new DaemonWsClient(url, {
      ...
      onSessionList: (sessions: WsSessionMeta[]) => {
        // Auto-attach to the first active session
      ...
      onRec: (rec: WsRec) => {
        ...
        // Dispatch to subscribers via store
        const cb = useSessionStore.getState()._onRec;
        cb?.(rec);
      },
