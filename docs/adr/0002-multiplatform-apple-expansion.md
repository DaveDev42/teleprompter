...
4. **watchOS = 별도 타깃, 제한적 경험.** `supportedDestinations` 가 아닌 독립
   ...
   재사용. **standalone RUNTIME**(자체 frontendId/kx, pairing secret 은 동기화된 iCloud
   Keychain — iPhone 과 떨어져 있어도 단독 구동, `WKRunsIndependentlyOfCompanionApp: YES`).
   ...
   > **DISTRIBUTION 정정 (ADR-0004 Amendment 2, #123, 2026-06-28):** watch 의 *런타임*
   > standalone 성격(이 항목)과 *타깃* 구조(별도 `TeleprompterWatch`)는 **그대로 유지**되나,
   > *배포* 모델만 바뀐다 — 별도 컨테이너(`TeleprompterWatchContainer`, 별도 ASC 레코드)가
   > 아니라 **메인 iOS 앱(`dev.tpmt.app`)에 임베드된 companion** 으로 배포한다(번들 ID 3→2,
   > ASC 레코드 2→1). 즉 companion DISTRIBUTION + standalone RUNTIME. 사용자는 iPhone 에서
   > 한 번 설정, 이후 watch 단독 구동. watch 번들 ID = `dev.tpmt.app.watchkitapp`,
   > `WKCompanionAppBundleIdentifier` = `dev.tpmt.app`(아래 B3 기록이 이미 이 값을 명시 —
   > 임베드 전 live `project.yml` 의 `dev.tpmt.app.watch` 는 드리프트였고 정정됨). 상세는
   > ADR-0004 §8.
...
- **OSLog 가시성:** 네이티브 macOS OSLog 는 String 변수 보간을 기본 `<private>` 로 가린다 →
  ...
  `log show/stream --predicate 'subsystem == "dev.tpmt.app"'` 로 스크랩.
...
## 5. Phase B 게이트 (B0) — ✅ 통과 (2026-06-15, Rust 1.96 stable)

```
rustup update stable   # → rustc 1.96 stable
rustup target add aarch64-apple-visionos aarch64-apple-visionos-sim
# 둘 다 성공 — prebuilt tier 2 아티팩트 제공됨
```
...
### B1 ✅ — TpCore.xcframework visionOS 슬라이스 추가 (PR #654, 2026-06-16)

- `build-xcframework.sh` 에 `aarch64-apple-visionos` (device) + `aarch64-apple-visionos-sim`
  (Simulator) 두 슬라이스 추가 → 5-slice xcframework (ios-device, ios-sim-fat,
  macos-fat, xros-arm64, xros-arm64-simulator).
- `plutil` LibraryIdentifier == 5 검증 통과.
- iOS 8/8, macOS 8/8 smoke 무회귀 확인.

### B2 ✅ — visionOS destination + spatial UX + harness (2026-06-16)

- `ios/project.yml`: `deploymentTarget.visionOS: "2.0"` + `supportedDestinations: [iOS, macOS, visionOS]`.
- `TeleprompterApp.swift`: visionOS `TabView { .glassBackgroundEffect() }` + `.defaultSize(960, 640)` spatial window.
- UIKit conditional import 정리: `NotificationService.swift`, `LiveRegion.swift`, `QRScannerView.swift`.
- `CALayer.autoresizingMask` 제거 (iOS 26.5 SDK 에서 삭제) → `updateUIView` 수동 resize.
- `scripts/ios.sh`: `TP_PLATFORM=visionos` 경로 추가 — `vision_sim_udid()` (xrOS 최신
  런타임 UDID 우선), UDID-based xcodebuild destination, `cmd_boot_visionos` /
  `cmd_build_visionos` / `cmd_smoke_visionos`.
- **iOS/visionOS 26.5 Simulator URL scheme 회귀 수정**: `xcrun simctl openurl` 이
  LaunchServices `-10814` 로 실패 → `xcrun simctl launch -- --tp-smoke-url <link>` +
...
- `ios/project.yml`: `deploymentTarget.watchOS: "10.0"` + 별도 `TeleprompterWatch` 타깃
  ...
  `WKCompanionAppBundleIdentifier: dev.tpmt.app`, `WKRunsIndependentlyOfCompanionApp: YES`).
