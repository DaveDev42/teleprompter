...
| **3** ◐ | interactive write — **3a ✅ session cleanup(multi-select TUI)** / **3b ☐ pair new**(QR 렌더 + OSC52 copy + ctrl+c→`pair.cancel` + pair-lock + spinner) | `crossterm` raw-mode(3a ✅); `qrcode`/`qr2term`(렌더만)·`fslock`/`fd-lock`(3b) | 3a: cleanup multi-select parity + non-TTY 거부 byte-parity + raw-mode 복원(RAII Drop) ✅; 3b: pair-blocking E2E parity(합성 kx frontend), QR 실앱 스캔 | medium |
...
- tranche 3b+ 는 잔여 A2.4 결정(#1 PTY spike, #3 locate, #4 relay trampoline, #5 cutover) 통과 후. tranche 3b=pair new(QR/OSC52/pair-lock/blocking, 합성 kx frontend E2E), 4=lifecycle thin, 5=run/passthrough(PTY spike).
