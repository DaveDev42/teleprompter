...
| 8b | **deploy pipeline** (pending) — `deploy-relay.yml` 을 Rust 바이너리로 전환: cross-compile `x86_64-unknown-linux-gnu` (Vultr 호스트 arch, `cross`/`cargo-zigbuild`), SCP `rust/target/x86_64-unknown-linux-gnu/release/tp-relay` → 호스트, systemd unit `ExecStart` 을 `/usr/local/bin/tp-relay`(별도 바이너리, `tp relay start` 아님) 로, `Environment=RELAY_PORT=7090`(이미 존재), `/health.buildSha==github.sha` assert(`build.rs` 가 `TP_BUILD_SHA` 컴파일타임 주입) | staging/canary 호스트에서 `/health` green + buildSha 일치 |
| 8c | **live cutover** (pending) — 아래 [CUTOVER RUNBOOK](#a17-cutover-runbook-8c) 순서대로 | dogfood: pair→chat→terminal→kill/reconnect(full-auth fallback)→push 재등록 |
...
> **8c 는 즉흥이 아니라 체크리스트다.** 8a(바이너리+로컬 게이트) ✅ + 8b(deploy pipeline) 가 green 인 뒤에만 실행. downtime-OK(Amendment 1) 이므로 in-flight resume/push 토큰은 **공유하지 않고 새로 발급** — 미결제 토큰은 verify 실패 → 기존 full-auth fallback 으로 흡수.

순서 (각 단계는 직전 단계 확인 후):

1. **announce + freeze** — dogfood 사용 일시 중단. relay 무중단 보장 없음(downtime-OK).
2. **stop TS relay** — 호스트에서 `systemctl stop tp-relay` (현행 `/usr/local/bin/tp relay start` 유닛).
3. **reissue secrets** — 새 `TP_RELAY_RESUME_SECRET`(≥32 char) + 새 `TP_RELAY_PUSH_SEAL_SECRET`(≥32 char) 생성, systemd `Environment=`(또는 EnvironmentFile) 에 주입. **이전 값과 공유하지 않는다** — 미결제 4-part TS resume 토큰은 verify 실패 → full-auth fallback. push-seal 회전이 아니라 clean reissue 이므로 `_PREV` 불필요(기존 sealed `push_tokens` 는 무효화, frontend 가 재등록).
4. **deploy Rust bin** — 8b 파이프라인(또는 수동 SCP) 로 `tp-relay` 바이너리 설치, systemd `ExecStart=/usr/local/bin/tp-relay`, `Environment=RELAY_PORT=7090`. `systemctl daemon-reload && systemctl start tp-relay`.
5. **verify /health.buildSha** — `curl https://relay.tpmt.dev/health` → `status:"ok"` + `buildSha == <배포 git sha>` (stale 바이너리 조기 검출). `protocolVersion:2`, `daemons:0` (아직 재연결 전).
6. **dogfood re-pair / reconnect** — 사용자 dogfood daemon(`~/.local/bin/tp`) 이 relay 에 재연결: 미결제 resume 토큰은 reject → full register+auth fallback 으로 자동 복구(코드 변경 불필요). 새 페어링이 필요하면 `tp pair new`. `/health daemons>=1` 확인.
7. **push re-register** — sealed `push_tokens` 가 새 push-seal 로 무효화됐으므로, frontend(앱)가 다음 relay 재연결 시 `relay.push.register` 로 1회 재등록(downtime 창에 정렬된 경계 있는 1회 누락). daemon `push_tokens` 행이 새 sealed blob 으로 갱신되는지 확인.
8. **smoke** — dogfood: pair→chat→terminal→kill/reconnect(full-auth fallback 경로 탐)→push 알림 1건 도착 확인. 이상 시 TS relay 로 롤백(유닛 `ExecStart` 복원 + 이전 secret — 단 reissue 후엔 양쪽 토큰셋이 갈라지므로 롤백도 full-auth fallback 을 한 번 더 탄다).
