...
| **1** | read-only ladder: completions/status/session list/pair list/logs (daemon-less rusqlite read) | `clap_complete`, `rusqlite`(read-only), ANSI | 각 커맨드 byte-parity + socket-path 파생 byte-exact + live daemon DB dogfood | low |
| **2** | write/control via daemon IPC: pair delete/rename, session delete/prune (IPC 1-왕복 + daemon-less fallback 결정) | `tokio` UnixStream, serde `IpcPair*`/`IpcSession*`/`Label`, `crossterm`(y/N 프롬프트) | IPC 프레임 골든벡터 vs `@teleprompter/protocol`; live daemon 왕복(peer `control.unpair`/`control.rename` 발화 확인); non-TTY 거부 parity | low |
| **3** | interactive write: pair new(QR 렌더 + OSC52 copy + ctrl+c→`pair.cancel` + pair-lock + spinner), session cleanup(multi-select TUI) | `qrcode`/`qr2term`(렌더만), `fslock`/`fd-lock`, `crossterm` raw-mode | pair-blocking E2E parity, cleanup multi-select parity, QR 실앱 스캔, raw-mode 복원 보장 | medium |
...
1. **PTY/runner spike 결과**: Rust `tp run` = thin forwarder(Rust PTY 없음) vs full Rust Runner(`portable-pty` vs `pty-process`). tranche 5 + Bun 제거를 블록.
2. **daemon-less write fallback**: pair/session write 시 daemon down 이면 rusqlite 직접 write 유지 vs daemon-up 강제(아키텍처상 단일 writer 에 더 깔끔). tranche 2 dep 표면(rusqlite write 권한) 결정.
...
- tranche 1+ 는 A2.4 결정(특히 #2 rusqlite write fallback) 통과 후.
