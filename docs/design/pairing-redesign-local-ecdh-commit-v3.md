...
| `ios/Sources/Pairing/DeepLinkHandler.swift` | `ingest` 호출 :38, `TP_PAIR_OK` 방출 :40 (마커 상수 :15, 하니스 `PAIR_MARKER` `scripts/ios.sh:88`, 어서션 4개 플랫폼 사이트 :917/:1257/:1491/:1729) | 마커 의미 재앵커: decode 성공 시점에는 `TP_PAIR_PENDING`(신규), `TP_PAIR_OK` 는 promote 시점으로 이동. smoke 하니스(`scripts/ios.sh`) 마커 목록 갱신 동반 (v2 §B.6 동일 방침). **real-daemon E2E 주의 (round-3 조건 12)**: `TP_E2E_REAL`/`TP_E2E_CLAUDE` 의 M0-M2 어서션 범위에서 kx 는 out-of-scope/racy (`.claude/rules/native-e2e-gates.md` "정직한 범위 — M0-M2 만") — promote 뒤로 이동한 `TP_PAIR_OK` 를 M1 로 계속 어서션하면 real-E2E 가 결정론적으로 깨진다. **PR-4 에서 real-E2E M1 어서션을 `TP_PAIR_PENDING` 으로 교체** (ingest 성공 = 종전 M1 과 동일 의미). loopback smoke(8마커) 는 kx 가 결정론적이므로 `TP_PAIR_OK` 를 계속 어서션하되 PR-4 단계 승격 조건(kx 완료)에 맞춰 통과 |
...
### 3.7 PR-6 ship 전 실기 검증 (2-device iCloud) — **수동 게이트 (CI 아님), PR-6 코드는 착지됨**

> **상태 (PR-6 구현 완료)**: Option A 저장 계층 (`PairingRecordStore` seam + synced whole-record
> blob + 레거시 마이그레이션 + macOS 런타임 프로브 + 포인터 인덱스 reconciliation)은 구현·유닛테스트·
> smoke 검증을 마치고 착지했다. 아래 5개 항목은 **iCloud 계정이 없는 CI 로는 재현 불가**하므로 PR 을
> 블록하지 않는다 (CI 는 loopback smoke + XCTest 로 결정론 검증만 함) — 대신 **App Store/TestFlight
> 실배포 전 2대의 실제 기기로 수행하는 수동 체크리스트**로 남긴다. 구현이 이 항목들을 **구조적으로**
> 지키도록 짜였음(각 항목 뒤 괄호의 코드 근거)을 유닛테스트가 프록시로 커버한다.

1. **서명된 macOS Release/TestFlight 빌드는 실제로 sync 되는가, 로컬 ad-hoc smoke 는 `-34018` 에서 로컬 전용으로 올바르게 강등되는가.** 서명 상태가 다른 두 실빌드를 각각 기기에서 구동해 확인한다. (구현: `PairingSyncProbe.syncAvailable` 런타임 프로브 — `errSecSuccess` 만 sync-on, 그 외 전부 local-only.)
2. **first-unlock 이전 cold-launch 열거가 `.locked`(빈 집합이 아니라)로 처리되고, unlock 후 retry 가 캐시를 비우지 않고 재충전하는가.** cold 상태 `SecItemCopyMatching(kSecMatchLimitAll)` 를 실기기에서 관찰. (구현: `loadAll` 이 `errSecItemNotFound` 만 `[]`, 그 외 non-success 는 `.locked` throw → `reconciledPointers` 가 last-good 유지; `PairingViewModel` 이 `protectedDataDidBecomeAvailableNotification` 관찰 → `reconnectAll` 재실행.)
3. **2-device 동시-추가(서로 다른 daemon)가 실제 iCloud 에서 무손실로 수렴하는가 (req-3 핵심 증명).** 같은 sync 창(한쪽/양쪽 잠깐 오프라인)에서 A 는 새 daemon 하나, B 는 다른 새 daemon 하나를 페어링 → **양쪽 기기에 두 페어링 모두 수렴**. (구현: per-pairingId Keychain account = item-granular merge; `reconciledPointers` 가 peer-arrival 을 append.)
4. **synced-delete 전파.** A 에서 Unpair → B 에도 아이템 삭제 전파되어 페어링 사라짐; A 에서 로컬 Remove → 디바이스-로컬 tombstone 만 (PR-7). (구현: `remove(daemonId:)` 가 blob + **레거시 synced secret** 을 삭제 = revocation; 마이그레이션은 레거시 secret 을 **절대 삭제 안 함** = peer 보존.)
5. **NEW — same-daemon 재페어링 수렴 (한쪽 오프라인 + 동시 재페어 둘 다).** A 에서 daemon-1 을 재페어(새 pairingId)한 뒤 B 온라인 → B 는 **엔트리 1개**, 새 relay/pk/pairingId 로 수렴하고 옛 orphan blob 이 제거되는가. 나아가 A·B 가 daemon-1 을 **동시에** 재페어한 경우에도(각자 다른 pairingId), 병합 후 모든 기기가 latest-`ts` blob 하나로 수렴하고 losing orphan 이 sweep 되는가. (구현: `persist` 는 save-before-sweep 로 durable, `reconciledPointers` 는 same-did 를 latest-`ts` 로 dedupe + **losing blob 을 `records.remove` 로 sweep** → ≤1-blob-per-did. 유닛: `testConcurrentRepairSweepsLosingOrphanBlob` / `testRepairReplacesSameDaemonBlobAndLoadsNewPairingId`.)

> **잔여 known-limitation (수용, PR-7 스코프)**: 한 기기가 **오프라인 상태에서** peer 의 unpair(blob synced-delete)를 놓치면, 그 기기의 로컬 포인터 맵은 삭제된 daemon 을 잠시 잔류시킨다 (`reconciledPointers` 는 열거 부재를 unpair 증거로 삼지 않는다 — 부재는 mid-sync 부분 열거와 구분 불가하므로 라이브 pairing 을 지우지 않는 쪽이 안전). 이는 (a) **라이브 `control.unpair` 푸시**(daemon→app, Keychain sync 무관)가 연결된 흔한 케이스를 커버하고, (b) 잔류 포인터는 blob 이 진짜 없으면 `load()` 가 `.notFound` → 행은 보이나 연결 안 됨(로컬 Remove 로 복구)이며, (c) 근본 해결(negative-evidence tombstone)은 PR-7 이 소유한다.
>
> **PR-7 착지 후 정정 (legacy 결정론)**: PR-7 의 local-hide tombstone 은 pairingId 키다. "숨긴 daemon 을 재-페어하면 새 pairingId 가 생겨 자동으로 다시 보인다" 는 **QR v4 (랜덤 UUID pairingId) 에서만 참**이다 — **legacy(v2/v3) daemon 은 `deriveLegacyPairingId(daemonId)` 가 daemonId 의 순수 함수(결정론적, `rust/tp-core/src/crypto.rs`)** 라 재-페어가 tombstone 이 지목하는 **같은 pairingId 를 그대로 re-mint** 한다. 그래서 tombstone 을 열거-부재로 GC 하지 않고 (hidden blob 은 계속 sync 되므로 부재하지 않는다), **오직 그 pairingId 의 deliberate recommit(persist/ingest) 또는 hard-delete(remove/unpair·reconcile loser sweep·orphan sweep) 시에만** clear 한다 — 그리고 recommit 은 incoming pairingId 뿐 아니라 `deriveLegacyPairingId(daemonId)` 도 함께 clear 해, legacy 재-페어가 stale tombstone 뒤에 영영 숨는 것을 막는다 (`unhideForDaemon`).
...
각 PR 은 독립 착지 가능(그린 상태 유지). **PR-1~PR-8 착지 완료 — 8-PR 재설계 종료.** (PR-6 = Option A, 사용자 greenlight 후 구현·검증·머지; PR-7 = unpair/local-hide split; PR-8 = `WS_PROTOCOL_VERSION` 2→3 + PCT/v4 문서.)
...
| **PR-6** | ✅ **착지** — Option A greenlit: `PairingRecordStore` seam + synced whole-record blob (`{ps,pk,relay,did,v,pairingId,hostname,ts}`) + 레거시 마이그레이션(secret 미삭제) + macOS 런타임 프로브 + daemonId→pairingId 포인터 인덱스 reconciliation(save-before-sweep durability, 부분-sync 포인터 보존, latest-`ts` dedupe + orphan sweep, retry-on-unlock). 실기 검증은 §3.7 수동 게이트. | `ios/Sources/Pairing/PairingRecordStore.swift`(신규 seam), `PairingStore.swift`(영속 계층 rewire), `TeleprompterApp.swift`(reconnect/unlock), `ios/Tests/PairingRecordStoreTests.swift`(14 테스트), `scripts/ios.sh`(smoke Keychain 정리) | PR-4; 사용자 greenlight ✅ |
| **PR-7** | ✅ **착지** — Unpair vs "이 기기에서만 제거" 구분 (v2 §E 승계). **device-local·NON-synced `localHidden` tombstone** (pairingId 키, `tp.pairing.<pid>.localHidden` + `tp.pairings.hidden` 인덱스, UserDefaults install-scoped). `hideLocally` 는 blob/secret 삭제도 `control.unpair` 발신도 **안 함** (=non-revoking) — synced blob 은 그대로 sync 되고 재설치 시 재-adopt. `reconciledPointers` 가 hidden pairingId 를 **loser-sweep 앞단에서** 필터(HIGH: resurrected hidden blob 이 latest-`ts` race 를 이겨 live re-pair 를 synced-delete 로 revoke 하는 것을 차단). **tombstone clear 는 recommit(persist/ingest)·hard-delete(remove/unpair·reconcile loser sweep·orphan sweep) 에서만** — 열거 부재로는 절대 clear 안 함(hidden blob 은 계속 sync 됨). `hideLocally` 는 sidecar(floor/PCT) 보존 + PENDING sweep(concurrent kx promote 부활 차단). **legacy(v2/v3) daemon 은 `deriveLegacyPairingId(daemonId)` 가 결정론적**이라 재-페어가 같은 pairingId 를 re-mint → recommit/reingest 가 **incoming + legacy-derived id 둘 다** unhide(안 하면 재-페어가 stale tombstone 뒤에 영영 숨음). UI = 2-버튼 confirm 시트(normal "이 기기에서만 제거" + red destructive "Unpair"). 인바운드 `control.unpair` 는 synced delete 유지(§E.3). smoke = `wipeAllCommittedForSmoke` 가 tombstone 도 clear. | `ios/Sources/Pairing/PairingStore.swift`(tombstone API·reconcile 필터·clear-triggers), `TeleprompterApp.swift`(`hideLocally` VM), `Nav/DaemonsTab.swift`(2-버튼 시트), `ios/Tests/PairingRecordStoreTests.swift`(+8 테스트), `ios/Tests/RelayResilienceTests.swift`(+1 VM 테스트), `scripts/ios.sh`(smoke tombstone 정리) | PR-4, PR-6 |
| **PR-8** | ✅ **착지** — 버전 게이트·문서: `WS_PROTOCOL_VERSION` 2→3 (`packages/protocol/src/compat.ts:43`; daemon kx 광고 = `relay-client.ts:592` `broadcastDaemonPublicKey` `v: WS_PROTOCOL_VERSION`, 앱 광고 = `RelayMessages.swift:9` `RelayProtocol.version` — **두 값 lockstep**). v3 bump 이 곧 광고 `v`=3 → 새-daemon+새-앱 페어에서 `effectiveV≥3` → PCT confirm 경로 활성(§1.3 표는 PR-5 에 이미 착지, 여기선 별도 hard-gate 코드 없음 — `pct` additive-optional, 판별 SoT = effectiveV+floor). 앱 테스트 3곳(`RelayAuthTests`/`RelayResilienceTests` 의 app-advertised `v` 어서션) 2→3; DaemonKxPayload **decode** 픽스처는 v2 유지(구-daemon backward-compat 커버). downgrade-safe 양방향 확인(구-앱은 `pct` 무시, 구-daemon 은 `v:3` frontend payload 를 `data.v` finite-number 로 무해 수용 `relay-client.ts:635`). 문서 = CLAUDE.md pairing bullet(+PCT/QR-v4) · ARCHITECTURE §5.6(PCT+버전게이트) · `.claude/rules/protocol.md`(kx `v` + WS-version bullet). | `packages/protocol/src/compat.ts`, `ios/Sources/Relay/RelayMessages.swift`, `ios/Tests/{RelayAuthTests,RelayResilienceTests}.swift`, CLAUDE.md, ARCHITECTURE.md, `.claude/rules/protocol.md` | PR-5 |
...
| 조건 12: loopback v:2 하드코드 | 확인 — `scripts/local-relay-loopback.ts:120, :131`. `TP_PAIR_OK` 상수 `DeepLinkHandler.swift:15`, 하니스 `PAIR_MARKER` `scripts/ios.sh:88` + 어서션 4사이트 :917/:1257/:1491/:1729; real-E2E 정직 범위 M0-M2 는 `.claude/rules/native-e2e-gates.md` 명문 |
