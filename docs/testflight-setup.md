...
> ## ✅ 완료됨 (2026-06-30 검증) — 이 체크리스트는 이력용 아카이브다
> **셋업은 끝났고 파이프라인은 라이브다.** 모든 GitHub Secret(13+개)이 2026-06-28 에 채워졌고,
> `TestFlight` 워크플로 run [`28418366127`](https://github.com/DaveDev42/teleprompter/actions/runs/28418366127)
> (태그 `v0.1.52`, 2026-06-30) 이 **iOS/macOS/visionOS 3플랫폼 전부 `altool ... No errors uploading`
> 로 success** — watch 슬라이스는 iOS `.ipa` 에 임베드되어 함께 업로드됨. 아래 체크박스는 그 완료 상태를
> 반영해 `[x]` 로 표시한다. 새 빌드를 올리려면 이 셋업을 다시 할 필요 없이 `/release` 로 새 `v*` 태그를
> 내거나 `gh workflow run testflight.yml -f tag=<태그>` 로 재-dispatch 하면 된다.
> **재검증 (2026-07-02)**: run [`28578680851`](https://github.com/DaveDev42/teleprompter/actions/runs/28578680851)
> (bare `main` HEAD `0f7567c` — 7개 UX 배치 포함, `workflow_dispatch`) 이 **iOS/macOS/visionOS 3플랫폼 전부
> `UPLOAD SUCCEEDED with no errors`** (빌드 iOS 1902 / macOS·visionOS 1901). iOS 첫 attempt 는
> `xcodebuild -downloadPlatform watchOS` 의 exit 70(Apple SDK CDN transient flake, 우리 코드 무관)으로만
> 실패해 실패-job 재실행(`gh run rerun --failed`)으로 통과 — 파이프라인 자체는 건강. 셋업은 여전히 라이브.
> **유일한 시한성 후속**: 배포 cert/profile 은 만료(보통 1년)되므로 lapse 시에만 계정 소유자가
> `scripts/setup-testflight-secrets.sh` 재실행으로 갱신.
...
- [x] **ASC API 키 발급** — App Store Connect → Users and Access → Integrations → App Store Connect
...
- [x] **ASC 앱 레코드** — 번들 ID `dev.tpmt.app` (iOS 플랫폼). 구 Expo 레코드 재사용 가능 (ADR-0004 §3).
- [x] **Apple Distribution 인증서**(.p12, cert+key) export →
      `gh secret set IOS_DIST_CERT_P12_BASE64 < <(base64 -i dist.p12)`
      `gh secret set IOS_DIST_CERT_PASSWORD -b '<p12 암호 (없으면 빈 문자열)>'`
- [x] **App Store provisioning profile** (`dev.tpmt.app`, Distribution) →
      `gh secret set IOS_PROVISIONING_PROFILE_BASE64 < <(base64 -i ios.mobileprovision)`
- [x] **검증**: `gh workflow run release.yml` 이후 `v*` 태그가 서면 `testflight.yml` 의 iOS job 이
...
- [x] **ASC 앱 레코드** — 번들 ID `dev.tpmt.app`, **macOS 플랫폼** (iOS 레코드와 별개 — ASC 는
      플랫폼별 레코드).
- [x] **Mac App Distribution 인증서**(.p12). iOS Distribution 과 **다른 인증서 타입**이다 →
      `gh secret set MAC_DIST_CERT_P12_BASE64 < <(base64 -i mac-dist.p12)`
      `gh secret set MAC_DIST_CERT_PASSWORD -b '<암호 (없으면 빈 문자열)>'`
- [x] **(방법 B 일 때만) Mac Installer Distribution 인증서**(별도 `.p12`) →
...
- [x] **Mac App Store provisioning profile** (`.provisionprofile`) →
      `gh secret set MAC_PROVISIONING_PROFILE_BASE64 < <(base64 -i mac.provisionprofile)`
- [x] **검증**: `TP_PLATFORM=macos TP_DEVELOPMENT_TEAM=<팀> scripts/ios.sh archive` 가 로컬 keychain
...
- [x] **ASC 앱 레코드** — 번들 ID `dev.tpmt.app`, **visionOS 플랫폼** (iOS/macOS 레코드와 별개).
- [x] **visionOS provisioning profile** (App Store, `dev.tpmt.app`, visionOS) →
...
- [x] **검증**: `testflight.yml` 의 visionOS job green. (`altool --type visionos` 는 Xcode 15.2+
      필요 — `macos-26` 러너 충족.)
...
- [x] **Apple Developer Portal — App ID 등록**: 번들 ID `dev.tpmt.app.watchkitapp` 를 Identifiers 에
      등록 (Capabilities: 필요한 것만; 구 `dev.tpmt.app.watch`/`dev.tpmt.app.watch.watchkitapp` 는 폐지).
- [x] **watch 앱 provisioning profile** (App Store Distribution, `dev.tpmt.app.watchkitapp`) →
      base64 로 인코딩해 GitHub Secret 에 등록:
      ```bash
      gh secret set IOS_WATCH_PROVISIONING_PROFILE_BASE64 < <(base64 -i watch-app.mobileprovision)
      ```
- [x] 인증서는 §1 의 `IOS_DIST_CERT_*` 재사용 (watchOS 전용 cert 타입 없음 — 추가 cert 시크릿 불필요).
- [x] **검증**: `testflight.yml` 의 iOS job (`guard` 의 `IOS_WATCH_PROVISIONING_PROFILE_BASE64` 포함
