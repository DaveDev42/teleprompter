...
test.describe("P0 — Full Roundtrip", () => {
      ...
      daemon.stderr?.on("data", (d) => {
        ...
        if (out.includes("session created")) {
          ...
          ws.onmessage = (e) => {
            ...
            if (msg.t === "hello" && msg.d.sessions.some((s: any) => s.sid === "roundtrip" && s.state === "running")) {
              ws.close();
              resolve();
            }
          };
  ...
  test("Terminal renders xterm with ANSI content from Claude", async ({ page }) => {
    await page.goto("/");
    ...
    for (let i = 0; i < 15; i++) {
      ...
      if ((await page.locator("body").textContent())?.includes("roundtrip")) break;
    }
    ...
    const hasRows = await page.locator(".xterm-rows").isVisible().catch(() => false);
    ...
    console.log(`xterm visible: ${xtermVisible}, rows visible: ${hasRows}, text length: ${xtermText.length}`);
  ...
  test("Chat shows PTY streaming content from Claude session", async ({ page }) => {
    await page.goto("/");
    await page.waitForSelector("text=Teleprompter", { timeout: 30_000 });
    ...
    const hasContent = bodyText.length > 200 &&
      !bodyText.includes("Waiting for session");

    console.log(`Session attached: ${hasSession}, content length: ${bodyText.length}`);
