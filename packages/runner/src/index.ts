...
async function gracefulShutdown(signal: string): Promise<void> {
  ...
  // Runner deliberately (e.g. `session.stop`/`session.restart` → killRunner),
  // not claude's own process exiting — reason "signal" so the daemon's
  // handleBye reports "stopped" rather than misreading the synthetic
  // 130/143 exit code as an "error".
