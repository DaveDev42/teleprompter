//! Runner CLI helpers: argv parsing + signal→exit-code mapping.
//!
//! Hoisted from `main.rs` into the library (ADR-0003 Phase 4, task #17 PR-1) so
//! the native `tp` passthrough path in `tp-cli` can reuse them verbatim — it
//! drives [`crate::runner::run`] in-process (the same lifecycle the daemon spawns
//! `tp-runner` for), constructing [`RunnerOptions`] via [`parse_args`] and using
//! [`wait_for_signal`] as the shutdown future. The `tp-runner` binary
//! (`main.rs`) is a thin wrapper over these.
...
pub fn parse_args(args: impl Iterator<Item = String>) -> Result<RunnerOptions, String> {
